...
function AdminClassManager({users}:{users:any[]}){
  ...
  const addMember=async(uid:number)=>{if(!selGroup)return;await supabase.from("class_members").insert({class_group_id:selGroup.id,user_id:uid}).catch(()=>{});fetchMembers(selGroup.id);};
