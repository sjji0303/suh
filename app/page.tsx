    ...
    <span className="text-[10px] text-slate-400">미제출</span>
      ...
      return(<div style={{position:"fixed",left:"-9999px",top:0,pointerEvents:"none"}}><div ref={capRef} style={{width:"520px",padding:"28px",background:"white",fontFamily:"'Apple SD Gothic Neo','Malgun Gothic','Noto Sans KR',sans-serif",WebkitFontSmoothing:"antialiased",MozOsxFontSmoothing:"grayscale",lineHeight:1.5,boxSizing:"border-box"}}>
        ...
        <div style={{background:"#f8fafc",borderRadius:"16px",padding:"14px",marginBottom:"14px"}}>
          ...
          {(()=>{
            const chartW=464;const barH=70;const labelH=28;const totalH=barH+labelH;const n=capQs.length;const barW=Math.max(8,Math.floor((chartW-(n-1)*3)/n));const gap=n>1?(chartW-barW*n)/(n-1):0;
            return(
                ...
                {capQs.map((q,i)=>{
                  ...
                  return(
                    <g key={q.question_number}>
                      ...
                      <text x={x+barW/2} y={barH+12} textAnchor="middle" fontSize={Math.min(9,barW)} fill="#64748b" fontWeight="600">{q.question_number}</text>
                      <text x={x+barW/2} y={barH+24} textAnchor="middle" fontSize={Math.min(8,barW-1)} fill="#94a3b8">{rate}%</text>
                    </g>
                  );
                })}
              </svg>
