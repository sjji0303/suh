...
function StudentView({user,logout}:{user:any;logout:()=>void}){
  ...
  return(<div className="min-h-screen bg-white flex">
    ...
    <main className="flex-1 lg:ml-56 pt-14 lg:pt-0"><div className="max-w-3xl mx-auto p-5 lg:p-8">
      {tab==="grades"&&<div>{test?<><div className="flex items-center justify-between mb-4"><button onClick={()=>nav(1)} className="p-2 hover:bg-slate-100 rounded-xl"><Icon type="left" size={20}/></button><div className="text-center"><p className="text-xl font-bold">{fmtDate(test.date)}</p><p className="text-sm text-slate-400">{test.title}</p></div><button onClick={()=>nav(-1)} className={`p-2 rounded-xl ${idx===0?"text-slate-200":"hover:bg-slate-100"}`} disabled={idx===0}><Icon type="right" size={20}/></button></div>
        ...
        {results.length>0?<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-slate-50 rounded-2xl p-5"><h3 className="font-semibold text-base mb-3">문항별 결과</h3><div className="space-y-1.5 max-h-80 overflow-y-auto">{questions.map(q=>(<div key={q.question_number} className="flex items-center gap-3 py-1"><span className="text-sm text-slate-400 w-6 text-right">{q.question_number}</span><span className="text-sm text-slate-500 flex-1 truncate">{q.topic||"—"}</span><span className={`text-sm font-bold w-7 text-center ${rm[q.question_number]?"text-blue-600":"text-red-400"}`}>{rm[q.question_number]?"O":"X"}</span><span className="text-xs text-slate-400 w-12 text-right">{q.correct_rate}%</span></div>))}</div></div>
            ...
            {rankHistory.length>=1&&(()=>{
              ...
              return(<div className="bg-slate-50 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-base">성적 추이</h3>
                  {prev&&diff!==0&&<span className={`text-sm font-bold px-3 py-1 rounded-lg ${diff>0?"bg-green-50 text-green-600":"bg-red-50 text-red-500"}`}>{diff>0?`▲ ${diff}단계 상승`:`▼ ${Math.abs(diff)}단계 하락`}</span>}
                  {prev&&diff===0&&<span className="text-sm font-bold px-3 py-1 rounded-lg bg-slate-100 text-slate-500">— 유지</span>}
