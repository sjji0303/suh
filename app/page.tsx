...
function StudentView({user,logout}:{user:any;logout:()=>void}){
  ...
  const[inquiries,setInquiries]=useState<any[]>([]);const[showInqAdd,setShowInqAdd]=useState(false);const[inqForm,setInqForm]=useState({title:"",content:""});
  ...
  const addInquiry=async()=>{if(!inqForm.content)return;await supabase.from("inquiries").insert({user_id:user.id,title:inqForm.title,content:inqForm.content});setInqForm({title:"",content:""});setShowInqAdd(false);const{data}=await supabase.from("inquiries").select("*").eq("user_id",user.id).order("created_at",{ascending:false});if(data)setInquiries(data);};
    ...
    <main className="flex-1 lg:ml-56 pt-14 lg:pt-0 pb-20 lg:pb-0"><div className="max-w-3xl mx-auto p-4 sm:p-5 lg:p-8">
      ...
      {tab==="inquiry"&&<div>
        ...
        {showInqAdd&&<div className="bg-slate-50 rounded-2xl p-5 mb-4 space-y-3">
          ...
          <div className="flex gap-2"><button onClick={addInquiry} className="bg-[#6c63ff] text-white px-4 py-2 rounded-xl text-xs font-semibold">등록</button><button onClick={()=>setShowInqAdd(false)} className="text-xs text-slate-400">취소</button></div>
        </div>}
        {inquiries.length>0?<div className="space-y-3">{inquiries.map((q:any)=>(<div key={q.id} className="bg-slate-50 rounded-2xl p-5"><div className="flex justify-between mb-1"><h3 className="font-semibold text-sm">{q.title||"문의"}</h3><span className="text-xs text-slate-400">{q.created_at?.slice(0,10)}</span></div><p className="text-sm text-slate-600 whitespace-pre-line">{q.content}</p>{q.reply&&<div className="mt-3 bg-[#6c63ff]/5 rounded-xl p-3"><p className="text-xs font-semibold text-[#6c63ff] mb-1">답변</p><p className="text-sm text-slate-700 whitespace-pre-line">{q.reply}</p></div>}</div>))}</div>:<div className="bg-slate-50 rounded-2xl p-12 text-center text-slate-400">문의사항이 없습니다</div>}
      </div>}
      {tab==="shop"&&<div>
...
function AdminInquiryManager(){
  const[inqs,setInqs]=useState<any[]>([]);const[replyId,setReplyId]=useState<number|null>(null);const[replyText,setReplyText]=useState("");
  ...
  useEffect(()=>{fI();},[]);
  const saveReply=async(id:number)=>{await supabase.from("inquiries").update({reply:replyText}).eq("id",id);setReplyId(null);setReplyText("");fI();};
  const delInq=async(id:number)=>{if(!confirm("삭제?"))return;await supabase.from("inquiries").delete().eq("id",id);fI();};
  return(<div><h2 className="text-lg font-bold mb-4">💬 문의사항 관리</h2>{inqs.length>0?<div className="space-y-3">{inqs.map((q:any)=>(<div key={q.id} className="bg-white rounded-2xl p-5 shadow-sm"><div className="flex justify-between mb-2"><div><span className="text-xs font-bold text-[#6c63ff] bg-[#6c63ff]/10 px-2 py-0.5 rounded-lg">{q.users?.name||"?"}</span><span className="text-xs text-slate-400 ml-2">{q.created_at?.slice(0,10)}</span></div><button onClick={()=>delInq(q.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div><h3 className="font-semibold text-sm mb-1">{q.title||"문의"}</h3><p className="text-sm text-slate-600 whitespace-pre-line mb-3">{q.content}</p>{q.reply?<div className="bg-[#6c63ff]/5 rounded-xl p-3"><p className="text-xs font-semibold text-[#6c63ff] mb-1">내 답변</p><p className="text-sm text-slate-700">{q.reply}</p><button onClick={()=>{setReplyId(q.id);setReplyText(q.reply);}} className="text-xs text-slate-400 mt-1">수정</button></div>:replyId===q.id?<div className="space-y-2"><textarea className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm border-0 resize-none h-20" value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="답변을 작성하세요"/><div className="flex gap-2"><button onClick={()=>saveReply(q.id)} className="bg-[#6c63ff] text-white px-4 py-1.5 rounded-xl text-xs font-semibold">답변</button><button onClick={()=>setReplyId(null)} className="text-xs text-slate-400">취소</button></div></div>:<button onClick={()=>{setReplyId(q.id);setReplyText("");}} className="text-xs text-[#6c63ff] font-semibold">답변하기</button>}</div>))}</div>:<div className="bg-white rounded-2xl p-12 shadow-sm text-center text-slate-400 text-sm">문의가 없습니다</div>}</div>);
}
