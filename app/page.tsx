...
function StudentView({user,logout}:{user:any;logout:()=>void}){
  ...
  const mis=[{id:"grades",icon:"test",label:"성적표"},{id:"settings",icon:"settings",label:"설정"}];
  return(<div className="min-h-screen bg-white flex">
    ...
    <main className="flex-1 lg:ml-56 pt-14 lg:pt-0"><div className="max-w-3xl mx-auto p-5 lg:p-8">
      {tab==="grades"&&<div>{test?<><div className="flex items-center justify-between mb-4"><button onClick={()=>nav(1)} className="p-2 hover:bg-slate-100 rounded-xl"><Icon type="left" size={18}/></button><div className="text-center"><p className="text-lg font-bold">{fmtDate(test.date)}</p><p className="text-xs text-slate-400">{test.title}</p></div><button onClick={()=>nav(-1)} className={`p-2 rounded-xl ${idx===0?"text-slate-200":"hover:bg-slate-100"}`} disabled={idx===0}><Icon type="right" size={18}/></button></div>
        ...
        {results.length>0?<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 rounded-2xl p-5"><h3 className="font-semibold text-sm mb-3">정답률</h3><div className="flex items-end gap-1 h-32">{questions.map(q=>{const rate=q.correct_rate||0;const isCorrect=rm[q.question_number];return(<div key={q.question_number} className="flex-1 flex flex-col items-center gap-1"><div className="w-full flex flex-col justify-end h-24 relative"><div className="w-full rounded-t transition-all" style={{height:`${Math.max(rate,4)}%`,background:isCorrect?"#6c63ff":"#ff6b6b"}}/></div><span className="text-[7px] text-slate-400 leading-none">{q.question_number}</span><span className="text-[6px] text-slate-400 leading-none">{rate}%</span></div>);})}</div></div>
            {wrong.length>0&&<div className="bg-slate-50 rounded-2xl p-5"><h3 className="font-semibold text-sm mb-4">최다 오답 TOP 3</h3><div className="flex justify-center gap-6">{wrong.slice(0,3).map((q:any)=>{const rate=q.correct_rate||0;const circumference=2*Math.PI*36;const filled=circumference*(rate/100);const empty=circumference-filled;return(<div key={q.question_number} className="flex flex-col items-center gap-2"><div className="relative w-20 h-20"><svg viewBox="0 0 80 80" className="w-full h-full -rotate-90"><circle cx="40" cy="40" r="36" fill="none" stroke="#f1f5f9" strokeWidth="6"/><circle cx="40" cy="40" r="36" fill="none" stroke="#ff6b6b" strokeWidth="6" strokeDasharray={`${filled} ${empty}`} strokeLinecap="round"/></svg><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-lg font-bold text-slate-700">{q.question_number}</span><span className="text-[9px] text-slate-400">번</span></div></div><div className="text-center"><p className="text-xs font-semibold text-red-400">{rate}%</p><p className="text-[10px] text-slate-400 max-w-[80px] truncate">{q.topic||"—"}</p></div></div>);})}</div></div>}
            ...
            {rankHistory.length>=2&&(()=>{
              // 등수 변화를 "성적 상승도"로 변환 (등수가 낮을수록 = 좋을수록 높게 표시)
              const data=rankHistory.map(h=>({date:h.date,value:h.total-h.rank+1,rank:h.rank,total:h.total}));
              const maxVal=Math.max(...data.map(d=>d.total));
              const w=320;const h=140;const px=40;const py=20;const gw=w-px*2;const gh=h-py*2;
              const points=data.map((d,i)=>{const x=px+(gw/(data.length-1))*i;const y=py+gh-(d.value/maxVal)*gh;return{x,y,...d};});
              const line=points.map((p,i)=>(i===0?"M":"L")+`${p.x},${p.y}`).join(" ");
              // 변화 화살표 계산
              const last=points[points.length-1];const prev=points[points.length-2];
              const diff=prev.rank-last.rank; // 양수면 등수 올라감(좋아짐)
              return(<div className="bg-slate-50 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-sm">성적 추이</h3>
                  {diff!==0&&<span className={`text-xs font-bold px-2 py-1 rounded-lg ${diff>0?"bg-green-50 text-green-600":"bg-red-50 text-red-500"}`}>{diff>0?`▲ ${diff}단계 상승`:`▼ ${Math.abs(diff)}단계 하락`}</span>}
                  {diff===0&&<span className="text-xs font-bold px-2 py-1 rounded-lg bg-slate-100 text-slate-500">— 유지</span>}
                </div>
                <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{maxHeight:"160px"}}>
                  ...
                  <path d={`${line} L${points[points.length-1].x},${py+gh} L${points[0].x},${py+gh} Z`} fill="url(#rankGrad)"/>
                  {/* 꺾은선 */}
                  <path d={line} fill="none" stroke="#6c63ff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                  {/* 점 + 날짜 */}
                  {points.map((p,i)=>(<g key={i}>
                    <circle cx={p.x} cy={p.y} r="4" fill="white" stroke="#6c63ff" strokeWidth="2"/>
                    <text x={p.x} y={h-2} textAnchor="middle" className="text-[8px]" fill="#94a3b8">{p.date.slice(5)}</text>
                  </g>))}
                </svg>
              </div>);
            })()}
