...
function AdminClassManager({users}:{users:any[]}){
  ...
  const captureReport=async(uid:number)=>{
    ...
    try{
      ...
      const canvas=await html2canvas(capRef.current,{backgroundColor:"#f0f2f8",scale:2,useCORS:true,logging:false,allowTaint:true,removeContainer:true,imageTimeout:0,onclone:(doc)=>{
        // html2canvas가 lab()/oklch() 등 최신 CSS 색상함수 미지원
        // cloned document의 모든 스타일시트에서 해당 색상 포함 규칙 제거
        Array.from(doc.styleSheets).forEach(ss=>{
          try{
            Array.from(ss.cssRules||[]).forEach((rule:any,i)=>{
              try{
                const txt=rule.cssText||"";
                if(txt.includes("lab(")||txt.includes("oklch(")||txt.includes("lch(")||txt.includes("color(")){
                  ss.deleteRule(i);
                }
              }catch{}
            });
