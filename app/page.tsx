...
function LoginScreen({onLogin,settings}:{onLogin:(id:string,pw:string)=>Promise<string>;settings:any}){
  ...
  const bg=settings.background_image&&settings.background_image.length>5?settings.background_image:"";const pi=settings.profile_image||"/profile.png";const nm=settings.profile_name||"서서갈비 T";const bio=(settings.profile_bio||"").split("\\n").join("\n");
        ...
        {tab==="home"&&(<div>
          ...
          {info&&<div className="bg-slate-50 rounded-2xl p-5 mb-4">
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              ...
              <div><p className="text-[10px] text-slate-400">과제 성취도</p><p className="text-sm font-bold text-slate-600">{info.assignment_score||"—"}</p></div>
            </div>
          </div>}
        </div>)}
