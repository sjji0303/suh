          ...
          {/* 3. 2단: 왼쪽 문항별 결과 / 오른쪽 점수+등수변화 */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
              ...
              <div className="space-y-5">
                {info&&<div className="ios-glass-card p-5 sm:p-6 relative z-10"><div className="grid grid-cols-2 gap-y-3 gap-x-3 text-center"><div><p className="grade-label">내 점수</p><p className="text-2xl font-bold leading-none tracking-tight" style={{color:"#D4AF37",textShadow:"0 2px 10px rgba(212,175,55,0.2)"}}>{info.total_score}<span className="text-sm sm:text-lg font-bold ml-1 text-slate-500">점</span></p></div><div><p className="grade-label">반 평균</p><p className="text-xl font-semibold leading-none tracking-tight mt-1" style={{color:"#3a3a5c"}}>{info.class_average}<span className="text-sm sm:text-base font-bold ml-1 text-slate-500">점</span></p></div><div className="mt-2"><p className="grade-label opacity-70">표준편차</p><p className="text-base font-medium tracking-tight mt-1" style={{color:"#8a8aaa"}}>{info.std_dev||"—"}<span className="text-[10px] sm:text-xs font-bold ml-1">{info.std_dev?"점":""}</span></p></div><div className="mt-2"><p className="grade-label opacity-70">최고 점수</p><p className="text-base font-medium tracking-tight mt-1" style={{color:"#8a8aaa"}}>{info.class_best}<span className="text-[10px] sm:text-xs font-bold ml-1">점</span></p></div></div></div>}
...
function AdminClassManager({users}:{users:any[]}){
  ...
  const saveAll=async()=>{
    ...
    if(errors.length>0)setSaveMsg("❌ "+errors.slice(0,3).join(" | "));
      ...
      try{
        ...
        if(autoEnabled){
          ...
          for(const m of members){
            ...
            const assignVal=parseFloat(String(inf.assignment_score||"0").replace(/[^0-9.]/g,""));
      ...
      try{for(const m of members){const uid=m.user_id;if(!hasA(uid))continue;const inf=ig[uid]||{};if((inf.clinic_time||"").startsWith("참석")){const ck=`자동:클리닉참석(test_${testId})`;const{data:dc}=await supabase.from("token_logs").select("id").eq("user_id",uid).eq("reason",ck).single();if(!dc){const{data:ud}=await supabase.from("users").select("tokens").eq("id",uid).single();await supabase.from("users").update({tokens:(ud?.tokens||0)+1}).eq("id",uid);await supabase.from("token_logs").insert({user_id:uid,amount:1,reason:ck});await sendNotif(uid,"token","🥩 클리닉 참석으로 서서갈비 1개 지급!");}}}
  ...
  if(selT&&selG){
    ...
    return(<div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-x-auto mb-4"><table className="text-xs border-collapse w-full"><thead><tr className="bg-slate-50"><th className="sticky left-0 bg-slate-50 z-10 px-3 py-2 text-left font-semibold text-slate-500 min-w-[80px]">이름</th><th className="px-2 py-2 font-semibold text-slate-500">출석</th><th className="px-2 py-2 font-semibold text-slate-500">클리닉</th><th className="px-2 py-2 font-semibold text-slate-500">과제 성취도</th><th className="px-2 py-2 font-semibold text-slate-500">오답 성취도</th><th className="px-2 py-2 font-semibold text-slate-500 min-w-[280px]">개인 코멘트</th></tr></thead><tbody>{members.map((m:any)=>{const uid=m.user_id;const usr=m.users;const inf=ig[uid]||{};return(<tr key={uid} className="border-b border-slate-50"><td className="sticky left-0 bg-white z-10 px-3 py-2 font-semibold"><div className="flex items-center gap-1">{usr?.login_id||usr?.name||"?"}{(inf.attendance==="출석"||inf.attendance==="영상")&&<button onClick={()=>captureReport(uid)} className="text-[9px] text-slate-300 hover:text-[#D4AF37] bg-slate-50 px-1.5 py-0.5 rounded">📷</button>}</div></td><td className="px-1 py-1"><select className="bg-slate-50 rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={inf.attendance||""} onChange={e=>setIC(uid,"attendance",e.target.value)}><option value="">—</option><option>출석</option><option>결석</option><option>영상</option></select></td><td className="px-1 py-1" style={{minWidth:"130px"}}><div className="flex flex-col gap-1"><label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" className="w-3 h-3 accent-[#D4AF37]" checked={(inf.clinic_time||"").startsWith("참석")} onChange={e=>{const t=(inf.clinic_time||"").replace("참석","").replace("불참","").replace(/^\s*\(|\)\s*$/g,"").trim();setIC(uid,"clinic_time",e.target.checked?"참석"+(t?" ("+t+")":""):"불참"+(t?" ("+t+")":""));}}/><span className="text-[10px] font-semibold text-slate-500">참석</span></label><input className="bg-slate-50 rounded-lg px-2 py-1 text-[10px] border-0 w-full" placeholder="금2:30-5:30" value={(inf.clinic_time||"").replace("참석","").replace("불참","").replace(/^\s*\(|\)\s*$/g,"").trim()} onChange={e=>{const isA=(inf.clinic_time||"").startsWith("참석");setIC(uid,"clinic_time",e.target.value?(isA?"참석":"불참")+" ("+e.target.value+")":(isA?"참석":"불참"));}}/></div></td><td className="px-1 py-1" style={{minWidth:"110px"}}><div className="flex flex-col gap-1"><input className="bg-slate-50 rounded-lg px-2 py-1.5 text-xs border-0 w-full" placeholder="90%" value={(inf.assignment_score||"").replace("+추가과제👍","").trim()} onChange={e=>{const hasB=(inf.assignment_score||"").includes("추가과제👍");setIC(uid,"assignment_score",e.target.value.trim()+(hasB?"+추가과제👍":""));}}/><label className="flex items-center gap-1 cursor-pointer px-0.5"><input type="checkbox" className="w-3 h-3 accent-[#D4AF37]" checked={(inf.assignment_score||"").includes("추가과제👍")} onChange={e=>{const s=(inf.assignment_score||"").replace("+추가과제👍","").trim();setIC(uid,"assignment_score",s+(e.target.checked?"+추가과제👍":""));}}/><span className="text-[10px] text-slate-400">추가과제👍</span></label></div></td><td className="px-1 py-1"><input className="bg-slate-50 rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={inf.wrong_answer_score||""} onChange={e=>setIC(uid,"wrong_answer_score",e.target.value)}/></td><td className="px-1 py-1"><textarea className="bg-slate-50 rounded-lg px-2 py-1.5 text-xs border-0 w-full resize-none h-16" value={inf.comment||""} onChange={e=>setIC(uid,"comment",e.target.value)} placeholder="개인 코멘트"/></td></tr>);})}</tbody></table></div>
      ...
      return(<div style={{position:"fixed",left:"-9999px",top:0,pointerEvents:"none"}}><div ref={capRef} style={{width:"520px",padding:"28px",background:"white",fontFamily:"'Apple SD Gothic Neo','Malgun Gothic','Noto Sans KR',sans-serif",WebkitFontSmoothing:"antialiased",MozOsxFontSmoothing:"grayscale",lineHeight:1.5,boxSizing:"border-box"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:"8px",background:"#f8fafc",borderRadius:"16px",padding:"14px",marginBottom:"14px"}}>
          ...
          <div style={{textAlign:"center"}}><p style={{fontSize:"10px",color:"#94a3b8",margin:0}}>과제 성취도</p><p style={{fontSize:"15px",fontWeight:"600",margin:"2px 0 0 0"}}>{(()=>{const r=inf.assignment_score||"";if(!r)return"—";const hasB=r.includes("추가과제👍");const num=r.replace("+추가과제👍","").replace(/%/g,"").trim();return num?(num+"%"+(hasB?"+추가과제👍":"")):"—";})()}</p></div>
