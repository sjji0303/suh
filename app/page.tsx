...
function StudentView({user,logout}:{user:any;logout:()=>void}){
  ...
  return(<div className="min-h-screen bg-white flex">
    <aside className="hidden lg:flex flex-col w-56 border-r border-slate-100 min-h-screen p-4 fixed left-0 top-0 bottom-0 z-40"><div className="flex items-center justify-between mb-6 px-2"><img src="/logo.png" alt="" className="h-7 object-contain"/><button onClick={()=>{setShowNotif(!showNotif);if(!showNotif)markAllRead();}} className="relative p-1"><Icon type="bell" size={18}/>{unreadCount>0&&<span className="absolute -top-1 -right-1 bg-red-500 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{unreadCount}</span>}</button></div><nav className="flex-1 space-y-1">{mis.map(m=>(<button key={m.id} onClick={()=>setTab(m.id)} className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium ${tab===m.id?"bg-slate-100 text-slate-900":"text-slate-500 hover:bg-slate-50"}`}><Icon type={m.icon} size={18}/>{m.label}</button>))}</nav><div className="border-t border-slate-100 pt-3"><div className="px-2 mb-2"><p className="text-xs font-semibold">{user.name}</p><p className="text-[10px] text-slate-400">{user.school||""}</p></div><button onClick={logout} className="flex items-center gap-2 w-full px-3 py-2 rounded-xl text-sm text-slate-400 hover:text-red-400 hover:bg-red-50"><Icon type="logout" size={16}/>로그아웃</button></div></aside>
    <div className="lg:hidden fixed top-0 left-0 right-0 bg-white border-b border-slate-100 z-40 px-4 py-3 flex justify-between items-center"><button onClick={()=>setMm(!mm)}><Icon type={mm?"close":"menu"} size={22}/></button><div className="flex items-center gap-2"><span className="text-xs font-semibold text-slate-600">{user.name}</span><button onClick={()=>{setShowNotif(!showNotif);if(!showNotif)markAllRead();}} className="relative p-1"><Icon type="bell" size={18}/>{unreadCount>0&&<span className="absolute -top-1 -right-1 bg-red-500 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{unreadCount}</span>}</button><img src="/logo.png" alt="" className="h-5"/></div></div>
    ...
    <main className="flex-1 lg:ml-56 pt-14 lg:pt-0"><div className="max-w-3xl mx-auto p-4 sm:p-5 lg:p-8">
      ...
      {tab==="myexam"&&<div>
        ...
        {showExamAdd&&<div className="bg-slate-50 rounded-2xl p-5 mb-4 space-y-4">
          {/* 시험 유형 선택 */}
          ...
          {examForm.exam_type==="모의고사"&&<>
            ...
            <div className="bg-white rounded-xl overflow-hidden border border-slate-200">
              <table className="w-full text-sm"><thead><tr className="bg-slate-50"><th className="px-3 py-2 text-xs font-semibold text-slate-400">과목</th><th className="px-3 py-2 text-xs font-semibold text-slate-400">국어</th><th className="px-3 py-2 text-xs font-semibold text-slate-400">수학</th><th className="px-3 py-2 text-xs font-semibold text-slate-400">영어</th><th className="px-3 py-2 text-xs font-semibold text-slate-400">과학</th><th className="px-3 py-2 text-xs font-semibold text-slate-400">사회</th></tr></thead><tbody>
              <tr className="border-t border-slate-100"><td className="px-3 py-2 text-xs font-semibold text-slate-500">등급</td>
                {[{k:"kor_grade"},{k:"math_grade"},{k:"eng_grade"},{k:"sci_grade"},{k:"soc_grade"}].map(c=>(<td key={c.k} className="px-1 py-1"><select className="w-full bg-slate-50 rounded-lg px-1 py-1.5 text-xs border-0 text-center" value={(examForm as any)[c.k]||""} onChange={e=>setExamForm(p=>({...p,[c.k]:e.target.value}))}><option value=""></option>{[1,2,3,4,5,6,7,8,9].map(g=>(<option key={g} value={`${g}`}>{g}</option>))}</select></td>))}
              </tr>
              <tr className="border-t border-slate-100"><td className="px-3 py-2 text-xs font-semibold text-slate-500">점수</td>
                {[{k:"kor_score"},{k:"math_score"},{k:"eng_score"},{k:"sci_score"},{k:"soc_score"}].map(c=>(<td key={c.k} className="px-1 py-1"><input className="w-full bg-slate-50 rounded-lg px-1 py-1.5 text-xs border-0 text-center" value={(examForm as any)[c.k]||""} onChange={e=>setExamForm(p=>({...p,[c.k]:e.target.value}))} placeholder=""/></td>))}
              </tr>
              </tbody></table>
            </div>
          </>}
        ...
        {myExams.length>0?<div className="space-y-3">{myExams.map((ex:any)=>{let memoObj:any={};try{memoObj=JSON.parse(ex.memo||"{}");}catch{}let subjects:any=null;try{if(ex.total&&ex.total.startsWith("{"))subjects=JSON.parse(ex.total);}catch{}return(<div key={ex.id} className="bg-slate-50 rounded-2xl p-4">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              {subjects?<div className="mt-2 bg-white rounded-xl overflow-hidden border border-slate-100"><table className="w-full text-xs"><thead><tr className="bg-slate-50"><th className="px-2 py-1.5 text-slate-400">과목</th>{["국어","수학","영어","과학","사회"].map(s=>(<th key={s} className="px-2 py-1.5 text-slate-400">{s}</th>))}</tr></thead><tbody><tr><td className="px-2 py-1 text-slate-500 font-semibold">등급</td>{["국어","수학","영어","과학","사회"].map(s=>(<td key={s} className="px-2 py-1 text-center font-bold text-[#6c63ff]">{subjects[s]?.grade||"—"}</td>))}</tr><tr className="border-t border-slate-50"><td className="px-2 py-1 text-slate-500 font-semibold">점수</td>{["국어","수학","영어","과학","사회"].map(s=>(<td key={s} className="px-2 py-1 text-center">{subjects[s]?.score||"—"}</td>))}</tr></tbody></table></div>:<><p className="font-semibold text-sm">{ex.subject}</p><div className="flex items-center gap-3 mt-1"><span className="text-lg font-bold text-[#6c63ff]">{ex.score}점</span>{ex.grade&&<span className="text-sm font-semibold text-slate-500">{ex.grade}</span>}</div></>}
