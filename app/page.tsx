...
function LoginScreen({onLogin,settings}:{onLogin:(id:string,pw:string)=>Promise<string>;settings:any}){
  ...
  return(
  <div className="min-h-screen relative flex flex-col overflow-hidden font-sans bg-[#050914]" onMouseMove={handleMouseMove}>
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&family=Pretendard:wght@300;400;500;600;700&display=swap');
      ...
      <div className={`hidden md:flex w-full max-w-5xl gap-8 transition-all duration-1000 ${ready?"opacity-100":"opacity-0"}`}>
        ...
        <div className="flex-1 glass-panel rounded-3xl p-12 flex flex-col justify-center fade-up delay-1 relative overflow-hidden">
          ...
          <div className="mb-10 relative z-10">
            <p className="text-[10px] text-[#D4AF37] font-semibold mb-3 tracking-[0.2em] uppercase font-sans">Exclusive</p>
      ...
      <div className="overflow-hidden mask-image-edges w-full max-w-[100vw]">
        <div className="flex gap-6 review-scroll pl-6 w-max">
          {[...reviews,...reviews,...reviews].map((r:any,i:number)=>{const hasBestGrade=r.best_grade&&r.best_grade.trim();return(<div key={i} className={`w-[280px] md:w-[320px] flex-shrink-0 glass-panel rounded-2xl p-6 transition-transform duration-500 hover:-translate-y-2 hover:scale-[1.02] ${hasBestGrade?"border-[#D4AF37]/30 bg-[#0B132B]/60":"border-white/5"}`}>
            ...
            {hasBestGrade&&<div className="mb-3"><span className="text-[10px] font-bold text-[#0B132B] bg-gradient-to-r from-[#D4AF37] to-[#F3E5AB] px-3 py-1 rounded-full shadow-[0_0_10px_rgba(212,175,55,0.3)]">🏆 {r.best_grade}</span></div>}
...
function StudentView({user,logout}:{user:any;logout:()=>void}){
  ...
  return(<div className="min-h-screen bg-gradient-to-br from-slate-50 via-[#F9FAFB] to-[#E2E8F0] flex">
    ...
    <main className="flex-1 lg:ml-64 pt-14 lg:pt-0"><div className="max-w-3xl mx-auto p-4 sm:p-5 lg:p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
            ...
            <div className="space-y-4">
              ...
              {rankHistory.length>=1&&(()=>{
                ...
                return(<div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 shadow-sm border border-slate-100/50">
      ...
      {tab==="myexam"&&<div>
        ...
        {showExamAdd&&<div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 mb-4 shadow-sm border border-slate-100/50 space-y-4">
          ...
          <div className="flex gap-2"><button onClick={addExam} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setShowExamAdd(false)} className="text-xs text-slate-400">취소</button></div>
        </div>}
      ...
      {tab==="notice"&&<div><h2 className="text-xl font-bold mb-4">📢 공지사항</h2>{notices.length>0?<div className="space-y-3">{notices.map((n:any)=>{const isNew=n.created_at&&(Date.now()-new Date(n.created_at).getTime())<24*60*60*1000;return(<div key={n.id} className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 shadow-sm border border-slate-100/50"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><h3 className="font-semibold text-base">{n.title||"공지"}</h3>{isNew&&<span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">N</span>}</div><div className="flex items-center gap-2"><span className="text-xs text-[#0B132B] bg-[#0B132B]/10 px-2 py-0.5 rounded-lg">{n.class_groups?.name||""}</span><span className="text-xs text-slate-400">{n.created_at?.slice(0,10)}</span></div></div><p className="text-sm text-slate-600 leading-relaxed whitespace-pre-line">{n.content}</p></div>);})}</div>:<div className="bg-white/60 rounded-2xl p-12 border border-slate-100/50 text-center text-slate-400">공지사항이 없습니다</div>}</div>}
      {tab==="inquiry"&&<div>
        ...
        {showInqAdd&&<div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 mb-4 shadow-sm border border-slate-100/50 space-y-3">
          ...
          <div className="flex gap-2"><button onClick={addInquiry} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">등록</button><button onClick={()=>{setShowInqAdd(false);setInqImg(null);}} className="text-xs text-slate-400">취소</button></div>
        </div>}
      ...
      {tab==="review"&&<div>
        <div className="flex justify-between items-center mb-5"><h2 className="text-xl font-bold">✍️ 수강 후기</h2>{!showReviewForm&&<button onClick={()=>setShowReviewForm(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-white px-4 py-2 rounded-2xl text-xs font-semibold shadow-md shadow-[#0B132B]/20">{myReview?"수정하기":"후기 작성"}</button>}</div>
        {showReviewForm?<div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-slate-100/50 space-y-5">
          <div><label className="text-sm font-semibold text-slate-700 mb-2 block">📈 성적이 제일 많이 올랐을 때는?</label><input className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm border-0 focus:ring-2 focus:ring-[#0B132B]/20 transition-all" value={reviewForm.best_grade} onChange={e=>setReviewForm(p=>({...p,best_grade:e.target.value}))} placeholder="예: 1학년 1학기 기말고사, 500등 → 100등"/><p className="text-[10px] text-slate-400 mt-1">예: 60점 → 100점, 혹은 500등 → 100등</p></div>
          <div><label className="text-sm font-semibold text-slate-700 mb-2 block">⭐ 서정인 쌤의 장점 (순위별 선택)</label>
            <div className="space-y-2">
              {[{label:"1순위",key:"kw1"},{label:"2순위",key:"kw2"},{label:"3순위",key:"kw3"}].map(r=>(<div key={r.key} className="flex items-center gap-3"><span className={`text-xs font-bold w-12 text-center py-1 rounded-lg ${r.key==="kw1"?"bg-amber-100 text-amber-600":r.key==="kw2"?"bg-slate-100 text-slate-500":"bg-orange-50 text-orange-400"}`}>{r.label}</span><select className="flex-1 bg-slate-50 rounded-xl px-4 py-2.5 text-sm border-0 focus:ring-2 focus:ring-[#0B132B]/20" value={(reviewForm as any)[r.key]} onChange={e=>setReviewForm(p=>({...p,[r.key]:e.target.value}))}><option value="">선택해주세요</option>{kwOptions.filter(kw=>kw===(reviewForm as any)[r.key]||![reviewForm.kw1,reviewForm.kw2,reviewForm.kw3].includes(kw)).map(kw=>(<option key={kw} value={kw}>#{kw}</option>))}</select></div>))}
            </div>
          </div>
          <div><label className="text-sm font-semibold text-slate-700 mb-2 block">💬 수강 후기</label><textarea className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm border-0 resize-none h-32 focus:ring-2 focus:ring-[#0B132B]/20 transition-all" value={reviewForm.content} onChange={e=>setReviewForm(p=>({...p,content:e.target.value}))} placeholder="서정인 수학을 수강하면서 느낀 점을 자유롭게 적어주세요"/></div>
          <div className="flex gap-2"><button onClick={saveReview} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-white px-6 py-2.5 rounded-xl text-sm font-semibold shadow-md shadow-[#0B132B]/20">저장</button><button onClick={()=>setShowReviewForm(false)} className="text-xs text-slate-400 px-4 py-2.5">취소</button></div>
        ...
        </div>:<div className="bg-white/60 rounded-2xl p-12 border border-slate-100/50 text-center"><p className="text-slate-400 text-sm mb-3">아직 후기를 작성하지 않았습니다</p><button onClick={()=>setShowReviewForm(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-white px-5 py-2 rounded-xl text-sm font-semibold shadow-md shadow-[#0B132B]/20">후기 작성하기</button></div>}
      </div>}
      {tab==="changepw"&&<div><h2 className="text-xl font-bold mb-4">🔒 비밀번호 변경</h2><div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-slate-100/50 max-w-md"><div className="space-y-3"><input type="password" className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm border-0 focus:ring-2 focus:ring-[#0B132B]/20" value={pw.n1} onChange={e=>setPw(p=>({...p,n1:e.target.value}))} placeholder="새 비밀번호"/><input type="password" className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm border-0 focus:ring-2 focus:ring-[#0B132B]/20" value={pw.n2} onChange={e=>setPw(p=>({...p,n2:e.target.value}))} placeholder="새 비밀번호 확인"/></div>{pwMsg&&<p className={`text-xs mt-2 ${pwMsg.includes("완료")?"text-green-500":"text-red-400"}`}>{pwMsg}</p>}<button onClick={chPw} className="mt-4 bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-white px-6 py-2.5 rounded-xl text-sm font-semibold shadow-md shadow-[#0B132B]/20">변경</button></div></div>}
    </div></main>
  </div>);
}
...
function AdminStudentManager({users,fetchUsers,groups}:{users:any[];fetchUsers:()=>void;groups:any[]}){
  ...
  return(<div>
    ...
    {showImport&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 border border-amber-200">
      ...
      <div className="flex gap-2 mt-3"><button onClick={importFromText} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">일괄 추가</button><button onClick={()=>setShowImport(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}

    {showAdd&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 border border-[#0B132B]/20">
      ...
      {groups.length>0&&<div className="mb-3"><label className="text-[10px] font-semibold text-slate-400">소속 반</label><div className="flex flex-wrap gap-2 mt-1">{groups.map(g=>(<button key={g.id} onClick={()=>toggleClass(g.id)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${form.class_ids.includes(g.id)?"bg-[#0B132B] text-white":"bg-slate-100 text-slate-500"}`}>{g.name}</button>))}</div></div>}
      {form.name&&form.parent_phone&&<p className="text-xs text-slate-400 mb-3">아이디: <b className="text-[#0B132B]">{form.name}{form.parent_phone.slice(-4)}</b> / 비번: <b>{form.parent_phone.slice(-4)}</b></p>}
      <div className="flex gap-2"><button onClick={addStudent} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">추가</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}
    ...
    {editStu&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 border border-amber-200">
      ...
      {editForm.name&&editForm.parent_phone&&<p className="text-xs text-slate-400 mb-3">변경될 아이디: <b className="text-[#0B132B]">{editForm.name}{editForm.parent_phone.slice(-4)}</b> / 비번: <b>{editForm.parent_phone.slice(-4)}</b></p>}
      <div className="flex gap-2"><button onClick={saveEditStu} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditStu(null)} className="text-xs text-slate-400">취소</button></div>
    </div>}

    <div className="bg-white rounded-2xl shadow-sm overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-50"><th className="px-3 py-3 text-left w-10"><input type="checkbox" checked={students.length>0&&checked.size===students.length} onChange={toggleAll} className="w-4 h-4 rounded border-slate-300 text-[#0B132B] cursor-pointer accent-[#0B132B]"/></th>{["이름","학교","아이디","학부모","학생",""].map(h=><th key={h} className="px-4 py-3 text-left text-xs font-semibold text-slate-400">{h}</th>)}</tr></thead><tbody>{students.map((s:any)=>(<tr key={s.id} className={`border-t border-slate-50 transition-colors ${checked.has(s.id)?"bg-red-50/50":"hover:bg-slate-50/50"}`}><td className="px-3 py-3"><input type="checkbox" checked={checked.has(s.id)} onChange={()=>toggleCheck(s.id)} className="w-4 h-4 rounded border-slate-300 text-[#0B132B] cursor-pointer accent-[#0B132B]"/></td><td className="px-4 py-3 font-semibold">{s.login_id||s.name}</td><td className="px-4 py-3 text-xs text-slate-500">{s.school||"—"}</td><td className="px-4 py-3 font-mono text-xs text-[#0B132B]">{s.login_id}</td><td className="px-4 py-3 text-xs text-slate-500">{s.parent_phone||"—"}</td><td className="px-4 py-3 text-xs text-slate-500">{s.student_phone||s.phone||"—"}</td><td className="px-4 py-3 text-right flex gap-2 justify-end"><button onClick={()=>startEdit(s)} className="text-xs text-slate-300 hover:text-[#0B132B]">수정</button><button onClick={()=>removeStudent(s.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></td></tr>))}{students.length===0&&<tr><td colSpan={7} className="text-center py-10 text-slate-400 text-sm">학생을 추가하세요</td></tr>}</tbody></table></div>
  </div>);
}
...
function AdminClassManager({users}:{users:any[]}){
  ...
  if(selG){
    ...
    return(<div>
      ...
      <div className="flex items-center gap-2 mb-4">{editingGId===selG.id?<div className="flex items-center gap-2"><input className="bg-slate-50 rounded-lg px-3 py-1.5 text-lg font-bold border border-slate-200 focus:outline-none focus:border-[#0B132B]" value={editGN} onChange={e=>setEditGN(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")renameG(selG.id);if(e.key==="Escape"){setEditingGId(null);setEditGN("");}}} autoFocus/><button onClick={()=>renameG(selG.id)} className="text-xs text-[#0B132B] font-semibold bg-[#0B132B]/10 px-3 py-1.5 rounded-lg">확인</button><button onClick={()=>{setEditingGId(null);setEditGN("");}} className="text-xs text-slate-400">취소</button></div>:<><h2 className="text-lg font-bold">📁 {selG.name}</h2><button onClick={()=>{setEditingGId(selG.id);setEditGN(selG.name);}} className="text-xs text-slate-400 hover:text-[#0B132B]">✏️ 수정</button></>}</div>
      <div className="bg-white rounded-2xl p-5 shadow-sm mb-4"><div className="flex justify-between items-center mb-3"><h3 className="font-semibold text-sm">학생 ({members.length}명)</h3><button onClick={()=>setShowAM(!showAM)} className="text-xs text-[#0B132B] font-semibold">+ 학생 추가</button></div><div className="flex flex-wrap gap-2">{members.map((m:any)=>(<div key={m.id} className="bg-slate-50 rounded-lg px-3 py-1.5 text-xs flex items-center gap-2"><span className="font-medium">{m.users?.login_id||m.users?.name}</span><button onClick={()=>rM(m.id)} className="text-red-300 hover:text-red-500">×</button></div>))}{members.length===0&&<p className="text-slate-400 text-xs">학생 추가 필요</p>}</div></div>
      ...
      <div className="flex justify-between items-center mb-3"><h3 className="font-semibold text-sm">시험</h3><button onClick={()=>{setShowNT(true);setNtf(p=>({...p,date:new Date().toISOString().split("T")[0]}));}} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">+ 새 시험</button></div>
      {showNT&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 flex gap-3 items-end"><div className="flex-1"><label className="text-xs font-semibold text-slate-500">시험 날짜</label><input type="date" className="w-full bg-slate-50 rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={ntf.date} onChange={e=>setNtf(p=>({...p,date:e.target.value}))}/></div><button onClick={cT} className="bg-[#0B132B] text-white px-5 py-2.5 rounded-xl text-xs font-semibold">생성</button><button onClick={()=>setShowNT(false)} className="text-xs text-slate-400 pb-1">취소</button></div>}
      <div className="space-y-2">{tests.map(t=>(<div key={t.id} className="bg-white rounded-xl p-4 shadow-sm hover:ring-2 hover:ring-[#0B132B]/30">
        {editTest?.id===t.id?<div className="space-y-3">
          ...
          <div className="flex gap-2"><button onClick={updateTest} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditTest(null)} className="text-xs text-slate-400">취소</button></div>
  ...
  return(<div>
    ...
    {showNG&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 flex gap-3 items-end"><div className="flex-1"><label className="text-xs font-semibold text-slate-500">반 이름</label><input className="w-full bg-slate-50 rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={newGN} onChange={e=>setNewGN(e.target.value)} placeholder="수학 정규반"/></div><button onClick={cG} className="bg-[#0B132B] text-white px-4 py-2.5 rounded-xl text-xs font-semibold">만들기</button></div>}
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{groups.map(g=>(<div key={g.id} className="bg-white rounded-xl p-5 shadow-sm hover:ring-2 hover:ring-[#0B132B]/20 cursor-pointer" onClick={()=>selectGroup(g)}><div className="flex justify-between items-start"><div className="flex items-center gap-2"><Icon type="folder" size={20}/>{editingGId===g.id?<div className="flex items-center gap-1" onClick={e=>e.stopPropagation()}><input className="bg-slate-50 rounded-lg px-2 py-1 text-sm border border-slate-200 focus:outline-none focus:border-[#0B132B] w-36" value={editGN} onChange={e=>setEditGN(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")renameG(g.id);if(e.key==="Escape"){setEditingGId(null);setEditGN("");}}} autoFocus/><button onClick={()=>renameG(g.id)} className="text-xs text-[#0B132B] font-semibold">확인</button><button onClick={()=>{setEditingGId(null);setEditGN("");}} className="text-xs text-slate-400">취소</button></div>:<span className="font-semibold">{g.name}</span>}</div><div className="flex gap-2" onClick={e=>e.stopPropagation()}><button onClick={()=>{setEditingGId(g.id);setEditGN(g.name);}} className="text-slate-300 hover:text-[#0B132B] text-xs">수정</button><button onClick={()=>dG(g.id)} className="text-slate-300 hover:text-red-400 text-xs">삭제</button></div></div></div>))}{groups.length===0&&<div className="bg-white rounded-2xl p-12 shadow-sm text-center text-slate-400 text-sm col-span-2">반을 만들어 보세요!</div>}</div>
  </div>);
}
...
function AdminNoticeManager({groups}:{groups:any[]}){
  ...
  return(<div>
    ...
    {showAdd&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 space-y-3">
      ...
      <div className="flex gap-2"><button onClick={addNotice} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">등록</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}
    <div className="space-y-3">{notices.map((n:any)=>(<div key={n.id} className="bg-white rounded-2xl p-5 shadow-sm">
      <div className="flex items-start justify-between mb-2"><div><h3 className="font-semibold text-sm">{n.title||"공지"}</h3><div className="flex items-center gap-2 mt-1"><span className="text-xs text-[#0B132B] bg-[#0B132B]/10 px-2 py-0.5 rounded-lg">{n.class_groups?.name||""}</span><span className="text-xs text-slate-400">{n.created_at?.slice(0,10)}</span></div></div><button onClick={()=>delNotice(n.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div>
...
function AdminExamViewer({users}:{users:any[]}){
  ...
  return(<div>
    ...
    {filtered.length>0?<div className="bg-white rounded-2xl shadow-sm overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-50">{["이름","학교","유형","구분","과목","점수","등급","성적변화","고민","하고싶은말"].map(h=><th key={h} className="px-3 py-3 text-left text-xs font-semibold text-slate-400 whitespace-nowrap">{h}</th>)}</tr></thead><tbody>{filtered.map((e:any)=>{let memoObj:any={};try{memoObj=JSON.parse(e.memo||"{}");}catch{}return(<tr key={e.id} className="border-t border-slate-50 hover:bg-slate-50/50"><td className="px-3 py-2.5 font-semibold whitespace-nowrap">{e.users?.name||"?"}</td><td className="px-3 py-2.5 text-xs text-slate-500">{e.users?.school||"—"}</td><td className="px-3 py-2.5"><span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${e.exam_type==="모의고사"?"bg-amber-50 text-amber-600":"bg-blue-50 text-blue-600"}`}>{e.exam_type}</span></td><td className="px-3 py-2.5 text-xs">{e.exam_name||"—"}</td><td className="px-3 py-2.5 text-xs">{e.subject}</td><td className="px-3 py-2.5 font-bold text-[#0B132B]">{e.score}점</td><td className="px-3 py-2.5 text-xs">{e.grade||"—"}</td><td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px]">{memoObj.q1||"—"}</td><td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px]">{memoObj.q2||"—"}</td><td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px]">{memoObj.q3||"—"}</td></tr>);})}</tbody></table></div>:<div className="bg-white rounded-2xl p-12 shadow-sm text-center text-slate-400 text-sm">학생이 입력한 성적이 없습니다</div>}
  </div>);
}
...
function AdminInquiryManager({onReply}:{onReply?:()=>void}){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">💬 문의사항 관리</h2>{inqs.length>0?<div className="space-y-3">{inqs.map((q:any)=>{const isNew=q.created_at&&(Date.now()-new Date(q.created_at).getTime())<24*60*60*1000;const hasReply=!!q.reply;const imgMatch=q.content?.match(/\[IMG\](.*?)\[\/IMG\]/);const cleanContent=q.content?.replace(/\[IMG\].*?\[\/IMG\]/g,"").trim();const replyImgMatch=q.reply?.match(/\[IMG\](.*?)\[\/IMG\]/);const cleanReply=q.reply?.replace(/\[IMG\].*?\[\/IMG\]/g,"").trim();return(<div key={q.id} className="bg-white rounded-2xl p-5 shadow-sm">
    <div className="flex justify-between mb-2"><div className="flex items-center gap-2"><span className="text-xs font-bold text-[#0B132B] bg-[#0B132B]/10 px-2 py-0.5 rounded-lg">{q.users?.name||"?"}</span><span className="text-xs text-slate-400">{q.created_at?.slice(0,10)}</span>{isNew&&<span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">N</span>}<span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${hasReply?"bg-green-50 text-green-600":"bg-amber-50 text-amber-600"}`}>{hasReply?"답변 완료":"답변 대기중"}</span></div><button onClick={()=>delInq(q.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div>
...
function ShortsGrid(){
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">{shorts.map((s:any)=>(<button key={s.id} onClick={()=>setPlaying(s.video_id)} className="group relative bg-black rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all hover:scale-[1.02]" style={{aspectRatio:"9/16"}}><img src={`https://img.youtube.com/vi/${s.video_id}/0.jpg`} alt={s.title} className="absolute inset-0 w-full h-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent"/><div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><div className="w-12 h-12 bg-white/90 rounded-full flex items-center justify-center shadow-lg"><svg viewBox="0 0 24 24" width="20" height="20" fill="#0B132B"><polygon points="8 5 20 12 8 19"/></svg></div></div>{s.title&&<p className="absolute bottom-2 left-2 right-2 text-[10px] text-white font-semibold leading-tight line-clamp-2">{s.title}</p>}</button>))}</div></>);
}
...
function AdminReviewViewer(){
  ...
  return(<div><div className="flex justify-between items-center mb-4"><h2 className="text-lg font-bold">✍️ 후기 관리 (로그인 화면 노출용)</h2><button onClick={()=>setShowAdd(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-md shadow-[#0B132B]/20 flex items-center gap-1"><Icon type="plus" size={14}/>후기 추가</button></div>
    {showAdd&&<div className="bg-white rounded-2xl p-5 shadow-sm mb-4 space-y-3">
      ...
      <div className="flex gap-2"><button onClick={addReview} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">추가</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}
    ...
    {reviews.length>0?<div className="space-y-3">{reviews.map((r:any)=>(<div key={r.id} className="bg-white rounded-2xl p-5 shadow-sm">
      {editId===r.id?<div className="space-y-3">
        ...
        <div className="flex gap-2"><button onClick={saveEdit} className="bg-[#0B132B] text-white px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditId(null)} className="text-xs text-slate-400">취소</button></div>
      </div>:<>
        <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><span className="text-xs font-bold text-[#0B132B] bg-[#0B132B]/10 px-2 py-0.5 rounded-lg">{r.display_name||"?"}</span><span className="text-xs text-slate-400">{r.display_school||""}</span></div><div className="flex items-center gap-2"><button onClick={()=>startEdit(r)} className="text-xs text-slate-300 hover:text-[#0B132B]">수정</button><button onClick={()=>delReview(r.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div></div>
        {r.best_grade&&<p className="text-xs text-amber-600 font-semibold mb-1">📈 {r.best_grade}</p>}
        {r.keywords&&<div className="flex flex-wrap gap-1 mb-2">{r.keywords.split(",").map((kw:string)=>(<span key={kw} className="bg-[#0B132B]/10 text-[#0B132B] px-2 py-0.5 rounded-full text-[10px] font-semibold">#{kw}</span>))}</div>}
...
function AdminStudentReviewViewer(){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">📝 학생 후기 ({reviews.length}건)</h2>
    {reviews.length>0?<div className="space-y-3">{reviews.map((r:any)=>(<div key={r.id} className="bg-white rounded-2xl p-5 shadow-sm"><div className="flex items-center gap-2 mb-3"><span className="text-xs font-bold text-[#0B132B] bg-[#0B132B]/10 px-2 py-0.5 rounded-lg">{r.users?.login_id||r.users?.name||"?"}</span><span className="text-xs text-slate-400">{r.users?.school||""}</span><span className="text-xs text-slate-300 ml-auto">{r.created_at?.slice(0,10)}</span><button onClick={()=>delStudentReview(r.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div>
...
function AdminShortsManager(){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">▶ 쇼츠 관리</h2>
    <div className="bg-white rounded-2xl p-5 shadow-sm mb-4 space-y-3"><p className="text-xs text-slate-400">유튜브 쇼츠 URL을 붙여넣기</p><div className="flex gap-2"><input className="flex-1 bg-slate-50 rounded-xl px-4 py-2.5 text-sm border-0" value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://youtube.com/shorts/xxxxx"/></div><div className="flex gap-2"><input className="flex-1 bg-slate-50 rounded-xl px-4 py-2.5 text-sm border-0" value={title} onChange={e=>setTitle(e.target.value)} placeholder="제목 (선택)"/><button onClick={addShort} className="bg-[#0B132B] text-white px-5 py-2.5 rounded-xl text-xs font-semibold">추가</button></div></div>
...
export default function Home(){
  ...
  const navEl=(mob?:boolean)=>(<nav className={`${mob?"":"flex-1"} space-y-1`}>
    {miPublic.map(m=>(<button key={m.id} onClick={()=>handleAdminTab(m.id,mob)} className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium transition-colors relative ${tab===m.id?"bg-[#0B132B] text-white":"text-slate-500 hover:bg-slate-50"}`}><Icon type={m.icon} size={18}/>{m.label}</button>))}
