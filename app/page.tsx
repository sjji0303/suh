...
function AdminTokenManager({users,fetchUsers}:{users:any[];fetchUsers:()=>void}){
  ...
  const fLogs=async()=>{const{data}=await supabase.from("token_logs").select("*, users:user_id(name)").order("created_at",{ascending:false}).limit(50);if(data)setLogs(data);};
  useEffect(()=>{fLogs();},[]);
  const giveToken=async(uid:number,subtract?:boolean)=>{const amt=Number(amount[uid]||0);if(!amt||amt<=0)return;const u=students.find(s=>s.id===uid);const cur=u?.tokens||0;const newVal=subtract?Math.max(0,cur-amt):cur+amt;await supabase.from("users").update({tokens:newVal}).eq("id",uid);await supabase.from("token_logs").insert({user_id:uid,amount:subtract?-amt:amt,reason:reason[uid]||(subtract?"관리자 차감":"관리자 지급")});setAmount(p=>({...p,[uid]:""}));setReason(p=>({...p,[uid]:""}));fetchUsers();fLogs();};
