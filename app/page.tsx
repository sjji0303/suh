...
function LoginScreen({onLogin,settings}:{onLogin:(id:string,pw:string)=>Promise<string>;settings:any}){
  ...
  return(<div className="min-h-screen relative flex flex-col overflow-hidden">
    ...
    {/* 메인 영역 — 위쪽으로 */}
    <div className="md:flex-1 flex items-start justify-center pt-15 md:pt-12 px-4 relative z-10">
      {/* PC */}
...
function AdminClassManager({users}:{users:any[]}){
  ...
  if(selT&&selG){
    ...
    return(<div>
      ...
      <div className="bg-white rounded-2xl shadow-sm mb-4 overflow-x-auto"><table className="text-xs border-collapse w-full"><thead><tr className="bg-slate-50"><th className="sticky left-0 bg-slate-50 z-10 px-3 py-2 text-left font-semibold text-slate-500 min-w-[80px]">이름</th><th className="px-2 py-2 font-semibold text-slate-500 min-w-[50px]">총점</th>{qs.map(q=><th key={q.question_number} className="px-1 py-2 font-semibold text-slate-400 min-w-[32px] text-center">{q.question_number}</th>)}</tr></thead><tbody>{members.map((m:any,ri:number)=>{const uid=m.user_id;const usr=m.users;const sc=getS(uid);const ans=hasA(uid);return(<tr key={uid} className="border-b border-slate-50"><td className="sticky left-0 bg-white z-10 px-3 py-2 font-semibold text-slate-700">{usr?.login_id||usr?.name||"?"}</td><td className="px-2 py-2 text-center font-bold text-[#6c63ff]">{ans?sc:"미응시"}</td>{qs.map((q,ci:number)=>{const k=`${uid}-${q.question_number}`;const v=grid[k];return(<td key={q.question_number} className="px-0.5 py-1 text-center"><input data-grid-row={ri} data-grid-col={ci} className={`w-7 h-7 text-center rounded font-bold text-xs border focus:outline-none focus:ring-1 focus:ring-[#6c63ff] ${v===1?"bg-blue-50 border-blue-200 text-blue-600":v===0?"bg-red-50 border-red-200 text-red-500":"bg-white border-slate-200"}`} value={v===undefined?"":v} onChange={e=>{const val=e.target.value;if(val===""||val==="0"||val==="1")setC(uid,q.question_number,val);}} onKeyDown={e=>{const ar={ArrowUp:[-1,0],ArrowDown:[1,0],ArrowLeft:[0,-1],ArrowRight:[0,1]}[e.key];if(!ar)return;e.preventDefault();const nr=ri+ar[0],nc=ci+ar[1];const next=document.querySelector(`input[data-grid-row="${nr}"][data-grid-col="${nc}"]`) as HTMLInputElement;if(next)next.focus();}} maxLength={1}/></td>);})}</tr>);})}</tbody></table></div>
