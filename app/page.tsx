  ...
  return(<div className="min-h-screen relative flex flex-col overflow-hidden">
    ...
    <div className="flex-1 flex items-start justify-center pt-6 md:pt-12 px-4 relative z-10">
      ...
      <div className={`md:hidden w-full max-w-xs transition-all duration-700 ${ready?"opacity-100 translate-y-0":"opacity-0 translate-y-8"}`}>
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-4 shadow-2xl border border-white/20 mb-3">
  ...
  return(<div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-[#6c63ff]/5 flex">
    <aside className="hidden lg:flex flex-col w-64 min-h-screen p-3 fixed left-0 top-0 bottom-0 z-40">
      <div className="flex-1 bg-white rounded-3xl shadow-lg border border-slate-100/80 p-5 flex flex-col m-2">
        <div className="flex items-center justify-between mb-8 px-1"><div className="bg-slate-50 rounded-2xl px-3 py-1.5"><img src="/logo.png" alt="" className="h-5 object-contain opacity-70"/></div><button onClick={()=>{setShowNotif(!showNotif);if(!showNotif)markAllRead();}} className="relative p-1.5 hover:bg-slate-100 rounded-xl transition-colors"><Icon type="bell" size={18}/>{unreadCount>0&&<span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{unreadCount}</span>}</button></div>
        <div className="bg-gradient-to-r from-[#6c63ff]/10 to-[#6c63ff]/5 rounded-2xl px-3.5 py-2.5 mb-4 flex items-center gap-2"><span className="text-base">🥩</span><div><p className="text-[10px] text-slate-400 font-semibold">서서갈비</p><p className="text-sm font-bold text-[#6c63ff]">{myTokens}개</p></div></div>
    ...
    <div className="lg:hidden fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-b border-slate-100/50 z-40 px-4 py-3 flex justify-between items-center shadow-sm"><button onClick={()=>setMm(!mm)} className="p-1 hover:bg-slate-100 rounded-xl transition-colors"><Icon type={mm?"close":"menu"} size={22}/></button><div className="flex items-center gap-2.5"><span className="text-[10px] font-bold text-[#6c63ff] bg-[#6c63ff]/10 px-2 py-1 rounded-lg">🥩 {myTokens}</span><span className="text-xs font-semibold text-slate-600">{user.name}</span><button onClick={()=>{setShowNotif(!showNotif);if(!showNotif)markAllRead();}} className="relative p-1 hover:bg-slate-100 rounded-xl transition-colors"><Icon type="bell" size={18}/>{unreadCount>0&&<span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{unreadCount}</span>}</button><div className="bg-slate-50 rounded-xl px-2 py-1"><img src="/logo.png" alt="" className="h-4 object-contain opacity-70"/></div></div></div>
