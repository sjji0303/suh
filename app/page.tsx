...
function LoginScreen({onLogin,settings}:{onLogin:(id:string,pw:string)=>Promise<string>;settings:any}){
  ...
  const go=async()=>{setLd(true);setErr(await onLogin(id.trim(),pw.trim()));setLd(false);};
  ...
  return(
  <div className="min-h-screen relative flex flex-col overflow-hidden bg-[#FDFBF7]" onMouseMove={handleMouseMove}>
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&display=swap');
      .font-serif { font-family: 'Playfair Display', serif; }
      .glass-panel { background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid rgba(212, 175, 55, 0.2); box-shadow: 0 30px 60px rgba(0,0,0,0.04), inset 0 1px 0 rgba(255,255,255,1); }
      .gold-gradient-text { background: linear-gradient(135deg, #C5A030 0%, #D4AF37 50%, #C5A030 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-size: 200% auto; animation: shine 4s linear infinite; }
      ...
      .shimmer-btn:hover { box-shadow: 0 10px 30px rgba(212, 175, 55, 0.3); border-color: rgba(212, 175, 55, 0.8); color: #F3E5AB; transform: translateY(-2px); }
      ...
      .luxury-input { background: rgba(255,255,255,0.9); border: 1px solid rgba(212,175,55,0.3); color: #0B132B; transition: all 0.3s ease; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02); }
      .luxury-input:focus { background: #FFFFFF; border-color: rgba(212,175,55,0.8); box-shadow: 0 0 15px rgba(212,175,55,0.15); outline: none; }
    ...
    <div className="absolute inset-0 z-0 scale-110 parallax-bg opacity-30" style={{backgroundImage:`url(${bg})`,backgroundSize:"cover",backgroundPosition:"center",filter:"blur(4px)",transform:`translate(${mousePos.x * 0.5}px, ${mousePos.y * 0.5}px) scale(1.05)`}}/>
    <div className="absolute inset-0 z-0 bg-gradient-to-b from-[#FDFBF7]/90 via-[#FDFBF7]/80 to-[#F5F7FA]"/>
    ...
    <div className="absolute -bottom-32 -left-32 w-96 h-96 bg-[#D4AF37]/10 rounded-full blur-[100px] z-0 parallax-bg pointer-events-none" style={{transform:`translate(${mousePos.x * -1}px, ${mousePos.y * -1}px)`}}/>
      ...
      <div className={`hidden md:flex w-full max-w-5xl gap-8 transition-all duration-1000 ${ready?"opacity-100":"opacity-0"}`}>
        <div className="w-[380px] glass-panel rounded-3xl p-8 flex-shrink-0 flex flex-col justify-center fade-up relative overflow-hidden">
          ...
          <div className="text-center mb-6 relative z-10">
            <div className="relative inline-block mb-6">
              <div className="absolute inset-0 bg-[#D4AF37] rounded-full blur-xl opacity-20 animate-pulse"/>
              <img src={pi} alt="" className="relative w-28 h-28 rounded-full shadow-lg object-cover border-[3px] border-[#D4AF37]/40"/>
            </div>
          ...
          {bio&&<div className="text-[13px] text-[#0B132B]/70 leading-relaxed whitespace-pre-line text-center px-4 font-light tracking-wide relative z-10">{bio}</div>}
          <div className="mt-8 pt-6 border-t border-[#0B132B]/10 text-center relative z-10"><p className="text-[11px] text-[#B59132] font-serif uppercase tracking-[0.3em] font-bold">Premium Education</p></div>
        </div>
        ...
        <div className="flex-1 glass-panel rounded-3xl p-12 flex flex-col justify-center fade-up delay-1 relative overflow-hidden">
          ...
          <div className="mb-10 relative z-10">
            <p className="text-[10px] text-[#B59132] font-bold mb-3 tracking-[0.2em] uppercase">Exclusive</p>
            <h1 className="text-4xl font-serif leading-tight font-light text-[#0B132B]"><span className="opacity-60">흐릿한 시작을, </span><br/><span className="font-bold gold-gradient-text">뚜렷한 선택으로.</span></h1>
          </div>
          <div className="space-y-5 max-w-sm relative z-10">
            ...
            <button onClick={go} disabled={ld} className="w-full shimmer-btn py-4 rounded-xl font-bold text-sm tracking-widest uppercase mt-4 shadow-[0_8px_20px_rgba(11,19,43,0.2)]">{ld?"AUTHENTICATING...":"SIGN IN"}</button>
          </div>
        </div>
      </div>
      ...
      <div className={`md:hidden w-full max-w-sm transition-all duration-1000 ${ready?"opacity-100":"opacity-0"}`}>
        <div className="glass-panel rounded-2xl p-6 mb-6 flex flex-col items-center fade-up relative overflow-hidden">
          ...
          <div className="relative mb-4 z-10">
            ...
            <img src={pi} alt="" className="relative w-20 h-20 rounded-full shadow-lg object-cover border-[2px] border-[#D4AF37]/40"/>
          </div>
          <h2 className="text-2xl font-serif font-bold gold-gradient-text tracking-wide text-center z-10">{nm}</h2>
          {bio&&<div className="text-[11px] text-[#0B132B]/70 leading-relaxed text-center mt-3 font-light px-2 z-10">{bio}</div>}
        </div>
        ...
        <div className="glass-panel rounded-2xl p-7 fade-up delay-1 relative overflow-hidden">
          ...
          <h1 className="text-2xl font-serif leading-tight font-light text-[#0B132B] mb-6 text-center z-10 relative"><span className="opacity-60">흐릿한 시작, </span><span className="font-bold gold-gradient-text">뚜렷한 선택</span></h1>
          ...
          <button onClick={go} disabled={ld} className="w-full shimmer-btn py-3.5 rounded-xl font-bold text-sm tracking-widest uppercase mt-6 relative z-10 shadow-[0_8px_20px_rgba(11,19,43,0.2)]">{ld?"WAIT...":"SIGN IN"}</button>
        </div>
      </div>
    </div>
    ...
    {reviews.length>0&&<div className={`relative z-10 pb-10 pt-6 transition-all duration-1000 delay-2 fade-up ${ready?"opacity-100":"opacity-0"}`}>
      <p className="text-center text-[#0B132B]/40 text-[10px] font-serif uppercase tracking-[0.3em] mb-6 font-bold">Stories of Excellence</p>
      <div className="overflow-hidden mask-image-edges w-full max-w-[100vw]">
        <div className="flex gap-6 review-scroll pl-6 w-max">
          {[...reviews,...reviews,...reviews].map((r:any,i:number)=>{const hasBestGrade=r.best_grade&&r.best_grade.trim();return(<div key={i} className={`w-[280px] md:w-[320px] flex-shrink-0 glass-panel rounded-2xl p-6 transition-transform duration-500 hover:-translate-y-2 hover:scale-[1.02] ${hasBestGrade?"border-[#D4AF37]/40 shadow-lg":"border-[#0B132B]/5"}`}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <span className="text-[13px] font-bold text-[#0B132B] block mb-1">{r.display_name||"익명"}</span>
                <span className="text-[10px] text-[#0B132B]/50">{r.display_school||""}</span>
              </div>
              <div className="text-[#B59132] text-lg opacity-80 font-serif">”</div>
            </div>
            {hasBestGrade&&<div className="mb-3"><span className="text-[10px] font-bold text-white bg-gradient-to-r from-[#D4AF37] to-[#B89326] px-3 py-1 rounded-full shadow-[0_4px_10px_rgba(212,175,55,0.3)]">🏆 {r.best_grade}</span></div>}
            {r.keywords&&<div className="flex flex-wrap gap-1.5 mb-3">{r.keywords.split(",").slice(0,3).map((kw:string,ki:number)=>(<span key={ki} className="text-[9px] font-medium text-[#0B132B]/60 border border-[#0B132B]/10 bg-[#0B132B]/5 px-2 py-0.5 rounded-full">#{kw}</span>))}</div>}
            <p className="text-[12px] text-[#0B132B]/80 leading-relaxed font-light review-clamp italic">{r.content}</p>
          </div>)})}
        </div>
      </div>
    </div>}
...
function StudentView({user,logout}:{user:any;logout:()=>void}){
  ...
  return(<div className="min-h-screen bg-gradient-to-br from-[#FDFBF7] via-white to-[#F5F7FA] flex">
    <aside className="hidden lg:flex flex-col w-64 min-h-screen p-3 fixed left-0 top-0 bottom-0 z-40">
      <div className="flex-1 bg-white/95 backdrop-blur-xl border border-[#D4AF37]/15 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] shadow-lg border border-slate-100/80 p-5 flex flex-col m-2">
        <div className="flex items-center justify-between mb-8 px-1"><div className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-2xl px-3 py-1.5"><img src="/logo.png" alt="" className="h-5 object-contain opacity-70"/></div><button onClick={()=>{setShowNotif(!showNotif);if(!showNotif)markAllRead();}} className="relative p-1.5 hover:bg-amber-50 rounded-xl transition-colors text-amber-500"><Icon type="bell" size={18}/>{unreadCount>0&&<span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{unreadCount}</span>}</button></div>
        <div className="px-2 mb-3 flex items-center gap-1.5"><span className="text-xs">🥩</span><span className="text-[11px] text-slate-400">{myTokens} 서서갈비</span></div>
        <nav className="flex-1 space-y-1">{mis.map(m=>(<button key={m.id} onClick={()=>setTab(m.id)} className={`flex items-center gap-3 w-full px-3.5 py-2.5 rounded-2xl text-sm font-medium transition-all ${tab===m.id?"bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] shadow-md shadow-[#D4AF37]/25":"text-slate-500 hover:bg-[#D4AF37]/10 hover:text-[#0B132B]"}`}><Icon type={m.icon} size={18}/>{m.label}</button>))}</nav>
        <div className="border-t border-slate-100 pt-4 mt-4"><div className="px-1 mb-2"><p className="text-xs font-semibold">{user.name}</p><p className="text-[10px] text-slate-400">{user.school||""}</p></div><button onClick={()=>setTab("changepw")} className="flex items-center gap-2 w-full px-3 py-2 rounded-2xl text-sm text-slate-400 hover:text-slate-600 hover:bg-[#D4AF37]/10 transition-colors"><Icon type="settings" size={16}/>비밀번호 변경</button><button onClick={logout} className="flex items-center gap-2 w-full px-3 py-2 rounded-2xl text-sm text-slate-400 hover:text-red-400 hover:bg-red-50 transition-colors"><Icon type="logout" size={16}/>로그아웃</button></div>
      </div>
    </aside>
    <div className="lg:hidden fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-2xl border-b border-[#D4AF37]/10 text-[#0B132B] border-b border-[#0B132B]/5 z-40 px-4 py-3 flex justify-between items-center shadow-sm"><button onClick={()=>setMm(!mm)} className="p-1 hover:bg-[#D4AF37]/20 rounded-xl transition-colors"><Icon type={mm?"close":"menu"} size={22}/></button><div className="flex items-center gap-2.5"><span className="text-[10px] text-slate-400">🥩 {myTokens}</span><span className="text-xs font-semibold text-slate-600">{user.name}</span><button onClick={()=>{setShowNotif(!showNotif);if(!showNotif)markAllRead();}} className="relative p-1 hover:bg-amber-50 rounded-xl transition-colors text-amber-500"><Icon type="bell" size={18}/>{unreadCount>0&&<span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{unreadCount}</span>}</button><div className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-2 py-1"><img src="/logo.png" alt="" className="h-4 object-contain opacity-70"/></div></div></div>
    {mm&&<><div onClick={()=>setMm(false)} className="lg:hidden fixed inset-0 bg-slate-50 backdrop-blur-sm z-40"/><div className="lg:hidden fixed left-0 top-2 bottom-2 w-64 bg-white/95 backdrop-blur-3xl z-50 border-r border-[#D4AF37]/20 text-[#0B132B] rounded-r-3xl shadow-2xl p-5 flex flex-col"><div className="flex justify-between items-center mb-6"><span className="font-bold">메뉴</span><button onClick={()=>setMm(false)}><Icon type="close" size={20}/></button></div><nav className="flex-1 space-y-1">{mis.map(m=>(<button key={m.id} onClick={()=>{setTab(m.id);setMm(false);}} className={`flex items-center gap-3 w-full px-3.5 py-2.5 rounded-2xl text-sm font-medium transition-all ${tab===m.id?"bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] shadow-md shadow-[#D4AF37]/25":"text-slate-500 hover:bg-[#D4AF37]/10"}`}><Icon type={m.icon} size={18}/>{m.label}</button>))}</nav><button onClick={()=>{setTab("changepw");setMm(false);}} className="flex items-center gap-2 w-full px-3 py-2.5 rounded-2xl text-sm text-slate-400 mt-2 hover:bg-[#D4AF37]/10 transition-colors"><Icon type="settings" size={16}/>비밀번호 변경</button><button onClick={()=>{logout();setMm(false);}} className="flex items-center gap-2 w-full px-3 py-2.5 rounded-2xl text-sm text-red-400 mt-1 hover:bg-red-50 transition-colors"><Icon type="logout" size={16}/>로그아웃</button></div></>}
    {/* 알림 패널 */}
    {showNotif&&<><div onClick={()=>setShowNotif(false)} className="fixed inset-0 bg-black/10 backdrop-blur-sm z-40"/><div className="fixed right-2 top-14 lg:left-14 lg:top-4 lg:right-auto w-80 max-h-[70vh] bg-white rounded-lg border border-[#D4AF37]/10 shadow-sm text-[#0B132B]/95 backdrop-blur-xl rounded-2xl shadow-2xl z-50 overflow-hidden border border-[#0B132B]/5"><div className="flex items-center justify-between px-4 py-3 border-b border-[#0B132B]/5 bg-gradient-to-r from-[#0B132B]/5 to-transparent"><h3 className="font-semibold text-sm">🔔 알림</h3><button onClick={()=>setShowNotif(false)} className="text-slate-400 hover:text-slate-600 transition-colors"><Icon type="close" size={16}/></button></div><div className="overflow-y-auto max-h-[60vh]">{notifs.length>0?notifs.map((n:any)=>(<div key={n.id} className={`px-4 py-3 border-b border-[#0B132B]/5 ${!n.is_read?"bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#B59132]":""}`}><div className="flex items-start gap-2"><div className="flex-1"><p className="text-sm text-[#0B132B]">{n.message}</p><p className="text-[10px] text-slate-400 mt-0.5">{n.created_at?.slice(0,10)} {n.created_at?.slice(11,16)}</p></div>{!n.is_read&&<span className="w-2 h-2 bg-red-500 rounded-full mt-1.5 flex-shrink-0"/>}</div></div>)):<div className="p-8 text-center text-slate-400 text-sm">알림이 없습니다</div>}</div></div></>}
    <main className="flex-1 lg:ml-64 pt-14 lg:pt-0"><div className="max-w-3xl mx-auto p-4 sm:p-5 lg:p-8">
      {tab==="grades"&&<div>{test?<><div className="flex items-center justify-between mb-2"><button onClick={()=>nav(1)} className="p-2 hover:bg-[#D4AF37]/20 rounded-xl"><Icon type="left" size={20}/></button><div className="text-center"><p className="text-xl font-bold">{fmtDate(test.date)}</p></div><div className="flex items-center gap-1"><button onClick={()=>nav(-1)} className={`p-2 rounded-xl ${idx===0?"text-slate-200":"hover:bg-[#D4AF37]/20"}`} disabled={idx===0}><Icon type="right" size={20}/></button></div></div>
        {/* 학생 정보 + 공유 */}
        <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><span className="text-xs text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2.5 py-1 rounded-lg font-semibold">{test.class_name||""}</span><span className="text-sm font-semibold text-[#0B132B]">{user.school||""} {user.name}</span></div><button onClick={async()=>{try{if(navigator.share){await navigator.share({title:`${user.name} 성적표 - ${test.title}`,text:`${user.name} | ${test.title}\n점수: ${info?.total_score||0}점 | 반평균: ${info?.class_average||0}점\n${window.location.href}`,});} else{await navigator.clipboard.writeText(`${user.name} | ${test.title}\n점수: ${info?.total_score||0}점 | 반평균: ${info?.class_average||0}점`);alert("성적 정보가 복사되었습니다!");}}catch{}}} className="text-xs text-slate-400 hover:text-[#B59132] flex items-center gap-1 bg-slate-50 border border-slate-100 text-[#0B132B] px-3 py-1.5 rounded-lg"><Icon type="upload" size={14}/>공유</button></div>
        {results.length>0?<>
          {/* 1. 출석/클리닉/과제/오답 성취도 */}
          {info&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-4 shadow-sm border border-[#0B132B]/5 mb-4 grid grid-cols-2 sm:grid-cols-4 gap-3"><div className="text-center"><p className="text-xs text-slate-400">출석</p><p className={`text-base font-bold ${info.attendance==="출석"?"text-green-600":info.attendance==="영상"?"text-amber-500":"text-red-500"}`}>{info.attendance||"—"}</p></div><div className="text-center"><p className="text-xs text-slate-400">클리닉</p><p className="text-base font-semibold text-slate-600">{info.clinic_time||"—"}</p></div><div className="text-center"><p className="text-xs text-slate-400">과제 성취도</p><p className="text-base font-semibold text-slate-600">{info.assignment_score?(String(info.assignment_score).replace(/%/g,"").trim()+"%"):"—"}</p></div><div className="text-center"><p className="text-xs text-slate-400">오답 성취도</p><p className="text-base font-semibold text-slate-600">{info.wrong_answer_score?(String(info.wrong_answer_score).replace(/%/g,"").trim()+"%"):"—"}</p></div></div>}
          {/* 2. 개인 코멘트 */}
          {info?.comment&&<div className="bg-gradient-to-r from-[#0B132B]/5 to-[#0B132B]/10 rounded-2xl p-5 mb-4 border border-[#D4AF37]/10"><p className="text-sm font-semibold text-[#B59132] mb-1">개인 코멘트</p><p className="text-base text-[#0B132B] leading-relaxed whitespace-pre-line">{info.comment}</p></div>}
          {/* 3. 2단: 왼쪽 문항별 결과 / 오른쪽 점수+등수변화 */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
            <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5"><h3 className="font-semibold text-base mb-3">문항별 결과</h3><div className="space-y-1.5">{questions.map(q=>(<div key={q.question_number} className="flex items-center gap-3 py-1"><span className="text-sm text-slate-400 w-6 text-right">{q.question_number}</span><span className="text-sm text-slate-500 flex-1 text-center">{q.topic||"—"}</span><span className={`text-sm font-bold w-7 text-center ${rm[q.question_number]?"text-blue-600":"text-red-400"}`}>{rm[q.question_number]?"O":"X"}</span><span className="text-xs text-slate-400 w-12 text-right">{q.correct_rate}%</span></div>))}</div></div>
            <div className="space-y-4">
              {info&&<div className="bg-gradient-to-br from-white to-[#0B132B]/5 backdrop-blur-sm rounded-2xl p-5 shadow-sm border border-[#D4AF37]/10"><div className="grid grid-cols-2 gap-3 text-center"><div><p className="text-xs text-slate-400">내 점수</p><p className="text-2xl font-bold text-[#B59132]">{info.total_score}<span className="text-sm font-semibold">점</span></p></div><div><p className="text-xs text-slate-400">반 평균</p><p className="text-2xl font-bold text-slate-600">{info.class_average}<span className="text-sm font-semibold">점</span></p></div><div><p className="text-xs text-slate-400">표준편차</p><p className="text-2xl font-bold text-slate-600">{info.std_dev||"—"}<span className="text-sm font-semibold">{info.std_dev?"점":""}</span></p></div><div><p className="text-xs text-slate-400">최고</p><p className="text-2xl font-bold text-slate-600">{info.class_best}<span className="text-sm font-semibold">점</span></p></div></div></div>}
              {rankHistory.length>=1&&(()=>{
                ...
                return(<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5">
          ...
          {/* 4. 하단 풀폭: 정답률 → 최다오답 */}
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5"><h3 className="font-semibold text-base mb-3">정답률</h3><div className="flex items-end gap-1 h-36">{questions.map(q=>{const rate=q.correct_rate||0;const isCorrect=rm[q.question_number];return(<div key={q.question_number} className="flex-1 flex flex-col items-center gap-1"><div className="w-full flex flex-col justify-end h-24 relative"><div className="w-full rounded-t transition-all" style={{height:`${Math.max(rate,4)}%`,background:isCorrect?"#0B132B":"#ff6b6b"}}/></div><span className="text-[9px] text-slate-500 leading-none font-semibold">{q.question_number}</span><span className="text-[8px] text-slate-400 leading-none">{rate}%</span></div>);})}</div></div>
            {wrong.length>0&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5"><h3 className="font-semibold text-base mb-4">최다 오답 TOP 3</h3><div className="flex justify-center gap-6">{wrong.slice(0,3).map((q:any)=>{const rate=q.correct_rate||0;const circumference=2*Math.PI*36;const filled=circumference*(rate/100);const empty=circumference-filled;return(<div key={q.question_number} className="flex flex-col items-center gap-2"><div className="relative w-22 h-22"><svg viewBox="0 0 80 80" className="w-20 h-20 -rotate-90"><circle cx="40" cy="40" r="36" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="6"/><circle cx="40" cy="40" r="36" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="6" strokeDasharray={`${filled} ${empty}`} strokeLinecap="round"/></svg><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-xl font-bold text-[#0B132B]">{q.question_number}</span><span className="text-[10px] text-slate-400">번</span></div></div><div className="text-center"><p className="text-sm font-semibold text-red-400">{rate}%</p><p className="text-xs text-slate-400 max-w-[80px] truncate">{q.topic||"—"}</p></div></div>);})}</div></div>}
          </div>
        </>:<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center text-slate-400 text-sm">결과 미입력</div>}
      </>:<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center text-slate-400">시험 없음</div>}</div>}
      {tab==="myexam"&&<div>
        <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold">📝 시험 결과</h2><button onClick={()=>setShowExamAdd(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-1 shadow-md shadow-[#D4AF37]/20 hover:shadow-lg transition-all"><Icon type="plus" size={14}/>성적 입력</button></div>
        {showExamAdd&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 mb-4 shadow-sm border border-[#0B132B]/5 space-y-4">
          {/* 시험 유형 선택 */}
          <div><label className="text-xs font-semibold text-slate-500">시험 유형</label><div className="flex gap-2 mt-1">{["모의고사","내신"].map(t=>(<button key={t} onClick={()=>setExamForm(p=>({...p,exam_type:t,exam_name:"",score:"",grade:""}))} className={`px-4 py-2 rounded-xl text-sm font-semibold ${examForm.exam_type===t?"bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)]":"bg-white text-slate-500 border border-slate-100"}`}>{t}</button>))}</div></div>

          {examForm.exam_type==="모의고사"&&<>
            <div><label className="text-xs font-semibold text-slate-500">몇월</label><select className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.exam_name} onChange={e=>setExamForm(p=>({...p,exam_name:e.target.value}))}><option value="">선택</option>{[3,4,6,7,9,10,11].map(m=>(<option key={m} value={`${m}월`}>{m}월</option>))}</select></div>
            {/* 과목별 표 */}
            <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] overflow-hidden border border-slate-100 shadow-sm">
              <table className="w-full text-xs"><thead><tr className="bg-gradient-to-r from-[#0B132B]/5 to-[#0B132B]/10"><th className="py-2.5 w-10 text-[10px] font-bold text-slate-500"></th>{["국어","수학","영어","과학","사회"].map(s=>(<th key={s} className="py-2.5 text-[11px] font-bold text-slate-600">{s}</th>))}</tr></thead><tbody>
              <tr className="border-t border-slate-100"><td className="py-2 text-center text-[10px] font-bold text-slate-400">등급</td>
                {[{k:"kor_grade"},{k:"math_grade"},{k:"eng_grade"},{k:"sci_grade"},{k:"soc_grade"}].map(c=>(<td key={c.k} className="py-1 px-0.5"><select className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg py-1.5 text-[11px] border-0 text-center font-semibold appearance-none" value={(examForm as any)[c.k]||""} onChange={e=>setExamForm(p=>({...p,[c.k]:e.target.value}))}><option value="">-</option>{[1,2,3,4,5,6,7,8,9].map(g=>(<option key={g} value={`${g}`}>{g}</option>))}</select></td>))}
              </tr>
              <tr className="border-t border-slate-100"><td className="py-2 text-center text-[10px] font-bold text-slate-400">점수</td>
                {[{k:"kor_score"},{k:"math_score"},{k:"eng_score"},{k:"sci_score"},{k:"soc_score"}].map(c=>(<td key={c.k} className="py-1 px-0.5"><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg py-1.5 text-[11px] border-0 text-center font-semibold" value={(examForm as any)[c.k]||""} onChange={e=>setExamForm(p=>({...p,[c.k]:e.target.value}))} placeholder="—"/></td>))}
              </tr>
              </tbody></table>
            </div>
          </>}
          ...
          {examForm.exam_type==="내신"&&<>
            <div><label className="text-xs font-semibold text-slate-500">시험 구분</label><select className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.exam_name} onChange={e=>setExamForm(p=>({...p,exam_name:e.target.value}))}><option value="">선택</option><option>1학기 중간</option><option>1학기 기말</option><option>2학기 중간</option><option>2학기 기말</option></select></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-semibold text-slate-500">수학 점수 *</label><input className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.score} onChange={e=>setExamForm(p=>({...p,score:e.target.value}))} placeholder="85"/></div>
              <div><label className="text-xs font-semibold text-slate-500">등급</label><select className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.grade} onChange={e=>setExamForm(p=>({...p,grade:e.target.value}))}><option value="">선택</option>{[1,2,3,4,5,6,7,8,9].map(g=>(<option key={g} value={`${g}등급`}>{g}등급</option>))}</select></div>
            </div>
          </>}

          {/* 공통 질문 */}
          <div><label className="text-xs font-semibold text-slate-500">수학 성적이 올랐나요?</label><input className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.q1} onChange={e=>setExamForm(p=>({...p,q1:e.target.value}))} placeholder="예: 저번보다 10점 올랐어요"/></div>
          <div><label className="text-xs font-semibold text-slate-500">공부에 가장 큰 고민은?</label><input className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.q2} onChange={e=>setExamForm(p=>({...p,q2:e.target.value}))} placeholder="예: 시간이 부족해요"/></div>
          <div><label className="text-xs font-semibold text-slate-500">하고 싶은 말</label><input className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={examForm.q3} onChange={e=>setExamForm(p=>({...p,q3:e.target.value}))} placeholder="자유롭게 적어주세요"/></div>
          <div className="flex gap-2"><button onClick={addExam} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setShowExamAdd(false)} className="text-xs text-slate-400">취소</button></div>
        </div>}
        {myExams.length>0?<div className="space-y-3">{myExams.map((ex:any)=>{let memoObj:any={};try{memoObj=JSON.parse(ex.memo||"{}");}catch{}let subjects:any=null;try{if(ex.total&&ex.total.startsWith("{"))subjects=JSON.parse(ex.total);}catch{}return(<div key={ex.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-4 shadow-sm border border-[#0B132B]/5">
        ...
        </div>);})}</div>:<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center text-slate-400">시험 성적을 입력해보세요</div>}
      </div>}
      {tab==="notice"&&<div><h2 className="text-xl font-bold mb-4">📢 공지사항</h2>{notices.length>0?<div className="space-y-3">{notices.map((n:any)=>{const isNew=n.created_at&&(Date.now()-new Date(n.created_at).getTime())<24*60*60*1000;return(<div key={n.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><h3 className="font-semibold text-base">{n.title||"공지"}</h3>{isNew&&<span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">N</span>}</div><div className="flex items-center gap-2"><span className="text-xs text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2 py-0.5 rounded-lg">{n.class_groups?.name||""}</span><span className="text-xs text-slate-400">{n.created_at?.slice(0,10)}</span></div></div><p className="text-sm text-slate-600 leading-relaxed whitespace-pre-line">{n.content}</p></div>);})}</div>:<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center text-slate-400">공지사항이 없습니다</div>}</div>}
      {tab==="inquiry"&&<div>
        ...
        {showInqAdd&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 mb-4 shadow-sm border border-[#0B132B]/5 space-y-3">
          <div><label className="text-xs font-semibold text-slate-500">제목</label><input className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={inqForm.title} onChange={e=>setInqForm(p=>({...p,title:e.target.value}))} placeholder="문의 제목"/></div>
          <div><label className="text-xs font-semibold text-slate-500">내용</label><textarea className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-3 text-sm mt-1 border border-slate-100 resize-none h-28" value={inqForm.content} onChange={e=>setInqForm(p=>({...p,content:e.target.value}))} placeholder="문의 내용을 적어주세요"/></div>
          <div><label className="text-xs font-semibold text-slate-500">이미지 첨부</label><div className="flex items-center gap-2 mt-1"><button onClick={()=>inqImgRef.current?.click()} className="bg-white rounded-lg border border-[#D4AF37]/10 shadow-sm text-[#0B132B] border border-slate-100 rounded-xl px-4 py-2 text-xs text-slate-500">{inqImg?inqImg.name:"이미지 선택"}</button><input ref={inqImgRef} type="file" accept="image/*" className="hidden" onChange={e=>{if(e.target.files?.[0])setInqImg(e.target.files[0]);}}/>{inqImg&&<button onClick={()=>setInqImg(null)} className="text-xs text-red-400">삭제</button>}</div></div>
          <div className="flex gap-2"><button onClick={addInquiry} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">등록</button><button onClick={()=>{setShowInqAdd(false);setInqImg(null);}} className="text-xs text-slate-400">취소</button></div>
        </div>}
        {inquiries.length>0?<div className="space-y-3">{inquiries.map((q:any)=>{const isNew=q.created_at&&(Date.now()-new Date(q.created_at).getTime())<24*60*60*1000;const hasReply=!!q.reply;const imgMatch=q.content?.match(/\[IMG\](.*?)\[\/IMG\]/);const cleanContent=q.content?.replace(/\[IMG\].*?\[\/IMG\]/g,"").trim();const replyImg=q.reply?.match(/\[IMG\](.*?)\[\/IMG\]/);const cleanReply=q.reply?.replace(/\[IMG\].*?\[\/IMG\]/g,"").trim();return(<div key={q.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5">
          {editInqId===q.id?<div className="space-y-3">
            <div><label className="text-xs font-semibold text-slate-500">제목</label><input className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-2.5 text-sm mt-1 border border-slate-100" value={editInqForm.title} onChange={e=>setEditInqForm(p=>({...p,title:e.target.value}))} placeholder="문의 제목"/></div>
            <div><label className="text-xs font-semibold text-slate-500">내용</label><textarea className="w-full bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-4 py-3 text-sm mt-1 border border-slate-100 resize-none h-28" value={editInqForm.content} onChange={e=>setEditInqForm(p=>({...p,content:e.target.value}))} placeholder="문의 내용"/></div>
            <div className="flex gap-2"><button onClick={saveEditInq} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditInqId(null)} className="text-xs text-slate-400">취소</button></div>
          </div>:<>
            <div className="flex justify-between mb-2"><div className="flex items-center gap-2"><span className="text-xs font-bold text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2 py-0.5 rounded-lg">{user.login_id||user.name}</span><h3 className="font-semibold text-sm">{q.title||"문의"}</h3>{isNew&&<span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">N</span>}<span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${hasReply?"bg-green-50 text-green-600":"bg-amber-50 text-amber-600"}`}>{hasReply?"답변 완료":"답변 대기중"}</span></div><div className="flex items-center gap-2"><span className="text-xs text-slate-400">{q.created_at?.slice(0,10)}</span>{!hasReply&&<button onClick={()=>startEditInq(q)} className="text-xs text-slate-300 hover:text-[#B59132]">수정</button>}<button onClick={()=>deleteInquiry(q.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div></div>
            <p className="text-sm text-slate-600 whitespace-pre-line">{cleanContent}</p>{imgMatch&&<img src={imgMatch[1]} alt="" className="mt-2 rounded-xl max-h-48 object-contain"/>}{hasReply&&<div className="mt-3 bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#B59132] rounded-xl p-3"><p className="text-xs font-semibold text-[#B59132] mb-1">답변</p><p className="text-sm text-[#0B132B] whitespace-pre-line">{cleanReply}</p>{replyImg&&<img src={replyImg[1]} alt="" className="mt-2 rounded-xl max-h-48 object-contain"/>}</div>}
          </>}
        </div>);})}</div>:<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center text-slate-400">문의사항이 없습니다</div>}
      </div>}
      {tab==="shop"&&<div>
        <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold">🏪 상점</h2><div className="bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-4 py-2 rounded-xl"><span className="text-sm font-bold text-[#B59132]">🥩 {myTokens} 서서갈비</span></div></div>
        {shopItems.length>0?<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">{shopItems.map((item:any)=>(<div key={item.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-5 shadow-sm border border-[#0B132B]/5"><h3 className="font-semibold text-base mb-1">{item.name}</h3>{item.description&&<p className="text-xs text-slate-400 mb-3">{item.description}</p>}<div className="flex items-center justify-between"><span className="text-sm font-bold text-[#B59132]">🥩 {item.price}</span><button onClick={()=>buyItem(item)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-1.5 rounded-xl text-xs font-semibold">구매</button></div></div>))}</div>:<div className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-2xl p-8 text-center text-slate-400 text-sm mb-6">상점 준비 중</div>}
        {purchases.length>0&&<div><h3 className="font-semibold text-sm mb-3">구매 내역</h3><div className="space-y-2">{purchases.map((p:any)=>(<div key={p.id} className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 flex justify-between items-center"><span className="text-sm">{p.shop_items?.name||"아이템"}</span><div className="text-right"><span className="text-xs font-bold text-[#B59132]">-{p.price} 🥩</span><p className="text-[10px] text-slate-400">{p.created_at?.slice(0,10)}</p></div></div>))}</div></div>}
      </div>}
      {tab==="shorts"&&<div>
      ...
      {tab==="review"&&<div>
        <div className="flex justify-between items-center mb-5"><h2 className="text-xl font-bold">✍️ 수강 후기</h2>{!showReviewForm&&<button onClick={()=>setShowReviewForm(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-4 py-2 rounded-2xl text-xs font-semibold shadow-md shadow-[#D4AF37]/20">{myReview?"수정하기":"후기 작성"}</button>}</div>
        {showReviewForm?<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-6 shadow-sm border border-[#0B132B]/5 space-y-5">
          <div><label className="text-sm font-semibold text-[#0B132B] mb-2 block">📈 성적이 제일 많이 올랐을 때는?</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm border-0 focus:ring-2 focus:ring-[#D4AF37]/20 transition-all" value={reviewForm.best_grade} onChange={e=>setReviewForm(p=>({...p,best_grade:e.target.value}))} placeholder="예: 1학년 1학기 기말고사, 500등 → 100등"/><p className="text-[10px] text-slate-400 mt-1">예: 60점 → 100점, 혹은 500등 → 100등</p></div>
          <div><label className="text-sm font-semibold text-[#0B132B] mb-2 block">⭐ 서정인 쌤의 장점 (순위별 선택)</label>
            <div className="space-y-2">
              {[{label:"1순위",key:"kw1"},{label:"2순위",key:"kw2"},{label:"3순위",key:"kw3"}].map(r=>(<div key={r.key} className="flex items-center gap-3"><span className={`text-xs font-bold w-12 text-center py-1 rounded-lg ${r.key==="kw1"?"bg-amber-100 text-amber-600":r.key==="kw2"?"bg-slate-100 text-slate-500":"bg-orange-50 text-orange-400"}`}>{r.label}</span><select className="flex-1 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm border-0 focus:ring-2 focus:ring-[#D4AF37]/20" value={(reviewForm as any)[r.key]} onChange={e=>setReviewForm(p=>({...p,[r.key]:e.target.value}))}><option value="">선택해주세요</option>{kwOptions.filter(kw=>kw===(reviewForm as any)[r.key]||![reviewForm.kw1,reviewForm.kw2,reviewForm.kw3].includes(kw)).map(kw=>(<option key={kw} value={kw}>#{kw}</option>))}</select></div>))}
            </div>
          </div>
          <div><label className="text-sm font-semibold text-[#0B132B] mb-2 block">💬 수강 후기</label><textarea className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm border-0 resize-none h-32 focus:ring-2 focus:ring-[#D4AF37]/20 transition-all" value={reviewForm.content} onChange={e=>setReviewForm(p=>({...p,content:e.target.value}))} placeholder="서정인 수학을 수강하면서 느낀 점을 자유롭게 적어주세요"/></div>
          <div className="flex gap-2"><button onClick={saveReview} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-6 py-2.5 rounded-xl text-sm font-semibold shadow-md shadow-[#D4AF37]/20">저장</button><button onClick={()=>setShowReviewForm(false)} className="text-xs text-slate-400 px-4 py-2.5">취소</button></div>
        </div>:myReview?<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-6 shadow-sm border border-[#0B132B]/5">
          {myReview.best_grade&&<div className="mb-4"><p className="text-xs font-semibold text-slate-400 mb-1">📈 성적 향상</p><p className="text-sm font-bold text-[#0B132B]">{myReview.best_grade}</p></div>}
          ...
          <div><p className="text-xs font-semibold text-slate-400 mb-1">💬 수강 후기</p><p className="text-sm text-[#0B132B] leading-relaxed whitespace-pre-line">{myReview.content}</p></div>
        ...
        </div>:<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center"><p className="text-slate-400 text-sm mb-3">아직 후기를 작성하지 않았습니다</p><button onClick={()=>setShowReviewForm(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-5 py-2 rounded-xl text-sm font-semibold shadow-md shadow-[#D4AF37]/20">후기 작성하기</button></div>}
      </div>}
      {tab==="changepw"&&<div><h2 className="text-xl font-bold mb-4">🔒 비밀번호 변경</h2><div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] rounded-2xl p-6 shadow-sm border border-[#0B132B]/5 max-w-md"><div className="space-y-3"><input type="password" className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm border-0 focus:ring-2 focus:ring-[#D4AF37]/20" value={pw.n1} onChange={e=>setPw(p=>({...p,n1:e.target.value}))} placeholder="새 비밀번호"/><input type="password" className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm border-0 focus:ring-2 focus:ring-[#D4AF37]/20" value={pw.n2} onChange={e=>setPw(p=>({...p,n2:e.target.value}))} placeholder="새 비밀번호 확인"/></div>{pwMsg&&<p className={`text-xs mt-2 ${pwMsg.includes("완료")?"text-green-500":"text-red-400"}`}>{pwMsg}</p>}<button onClick={chPw} className="mt-4 bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-6 py-2.5 rounded-xl text-sm font-semibold shadow-md shadow-[#D4AF37]/20">변경</button></div></div>}
    </div></main>
  </div>);
}
...
function AdminStudentManager({users,fetchUsers,groups}:{users:any[];fetchUsers:()=>void;groups:any[]}){
  ...
  return(<div>
    ...
    {showImport&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 border border-amber-200">
      ...
      <textarea className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-xs border-0 resize-none h-32 font-mono focus:outline-none" value={importText} onChange={e=>setImportText(e.target.value)} placeholder={"김민서\t경기여고\t01012345678\t01098765432\n박지성\t단대부고\t01011112222\t01033334444"}/>
      <div className="flex gap-2 mt-3"><button onClick={importFromText} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">일괄 추가</button><button onClick={()=>setShowImport(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}

    {showAdd&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 border border-[#D4AF37]/20">
      <h3 className="font-semibold text-sm mb-3">새 학생</h3>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div><label className="text-[10px] font-semibold text-slate-400">이름 *</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div>
        <div><label className="text-[10px] font-semibold text-slate-400">학교</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={form.school} onChange={e=>setForm(p=>({...p,school:e.target.value}))}/></div>
        <div><label className="text-[10px] font-semibold text-slate-400">학부모 연락처 *</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={form.parent_phone} onChange={e=>setForm(p=>({...p,parent_phone:e.target.value}))} placeholder="01064382222"/></div>
        <div><label className="text-[10px] font-semibold text-slate-400">학생 연락처</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={form.student_phone} onChange={e=>setForm(p=>({...p,student_phone:e.target.value}))}/></div>
      </div>
      {groups.length>0&&<div className="mb-3"><label className="text-[10px] font-semibold text-slate-400">소속 반</label><div className="flex flex-wrap gap-2 mt-1">{groups.map(g=>(<button key={g.id} onClick={()=>toggleClass(g.id)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${form.class_ids.includes(g.id)?"bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)]":"bg-slate-100 text-slate-500"}`}>{g.name}</button>))}</div></div>}
      {form.name&&form.parent_phone&&<p className="text-xs text-slate-400 mb-3">아이디: <b className="text-[#B59132]">{form.name}{form.parent_phone.slice(-4)}</b> / 비번: <b>{form.parent_phone.slice(-4)}</b></p>}
      <div className="flex gap-2"><button onClick={addStudent} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">추가</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}

    {editStu&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 border border-amber-200">
      <h3 className="font-semibold text-sm mb-3">✏️ 학생 정보 수정 — {editStu.name}</h3>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div><label className="text-[10px] font-semibold text-slate-400">이름 *</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={editForm.name} onChange={e=>setEditForm(p=>({...p,name:e.target.value}))}/></div>
        <div><label className="text-[10px] font-semibold text-slate-400">학교</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={editForm.school} onChange={e=>setEditForm(p=>({...p,school:e.target.value}))}/></div>
        <div><label className="text-[10px] font-semibold text-slate-400">학부모 연락처 *</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={editForm.parent_phone} onChange={e=>setEditForm(p=>({...p,parent_phone:e.target.value}))}/></div>
        <div><label className="text-[10px] font-semibold text-slate-400">학생 연락처</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-2 text-sm mt-1 border-0" value={editForm.student_phone} onChange={e=>setEditForm(p=>({...p,student_phone:e.target.value}))}/></div>
      </div>
      {editForm.name&&editForm.parent_phone&&<p className="text-xs text-slate-400 mb-3">변경될 아이디: <b className="text-[#B59132]">{editForm.name}{editForm.parent_phone.slice(-4)}</b> / 비번: <b>{editForm.parent_phone.slice(-4)}</b></p>}
      <div className="flex gap-2"><button onClick={saveEditStu} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditStu(null)} className="text-xs text-slate-400">취소</button></div>
    </div>}

    <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-50 border border-slate-100 text-[#0B132B]"><th className="px-3 py-3 text-left w-10"><input type="checkbox" checked={students.length>0&&checked.size===students.length} onChange={toggleAll} className="w-4 h-4 rounded border-slate-300 text-[#B59132] cursor-pointer accent-[#0B132B]"/></th>{["이름","학교","아이디","학부모","학생",""].map(h=><th key={h} className="px-4 py-3 text-left text-xs font-semibold text-slate-400">{h}</th>)}</tr></thead><tbody>{students.map((s:any)=>(<tr key={s.id} className={`border-t border-[#0B132B]/5 transition-colors ${checked.has(s.id)?"bg-red-50/50":"hover:bg-[#D4AF37]/10"}`}><td className="px-3 py-3"><input type="checkbox" checked={checked.has(s.id)} onChange={()=>toggleCheck(s.id)} className="w-4 h-4 rounded border-slate-300 text-[#B59132] cursor-pointer accent-[#0B132B]"/></td><td className="px-4 py-3 font-semibold">{s.login_id||s.name}</td><td className="px-4 py-3 text-xs text-slate-500">{s.school||"—"}</td><td className="px-4 py-3 font-mono text-xs text-[#B59132]">{s.login_id}</td><td className="px-4 py-3 text-xs text-slate-500">{s.parent_phone||"—"}</td><td className="px-4 py-3 text-xs text-slate-500">{s.student_phone||s.phone||"—"}</td><td className="px-4 py-3 text-right flex gap-2 justify-end"><button onClick={()=>startEdit(s)} className="text-xs text-slate-300 hover:text-[#B59132]">수정</button><button onClick={()=>removeStudent(s.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></td></tr>))}{students.length===0&&<tr><td colSpan={7} className="text-center py-10 text-slate-400 text-sm">학생을 추가하세요</td></tr>}</tbody></table></div>
  </div>);
}
...
function AdminClassManager({users}:{users:any[]}){
  ...
  if(selT&&selG){
    ...
    return(<div>
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2"><div><h2 className="text-lg font-bold">{selT.title}</h2><p className="text-xs text-slate-400">{fmtDate(selT.date)} · 과제: {selT.assignment||"없음"}</p></div><div className="flex items-center gap-3"><button onClick={saveAll} disabled={saving} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-6 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50">{saving?"저장 중...":"💾 전체 저장"}</button>{saveMsg&&<span className={`text-sm font-semibold ${saveMsg.includes("완료")?"text-green-500":"text-red-500"}`}>{saveMsg}</span>}</div></div>
      <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm mb-4 overflow-x-auto" onMouseUp={onCellMouseUp} style={{userSelect:"none"}}><table className="text-xs border-collapse w-full"><thead><tr className="bg-slate-50 border border-slate-100 text-[#0B132B]"><th className="sticky left-0 bg-slate-50 border border-slate-100 text-[#0B132B] z-10 px-3 py-2 text-left font-semibold text-slate-500 min-w-[80px]">이름</th><th className="px-2 py-2 font-semibold text-slate-500 min-w-[50px]">총점</th>{qs.map(q=><th key={q.question_number} className="px-1 py-2 font-semibold text-slate-400 min-w-[32px] text-center">{q.question_number}</th>)}</tr></thead><tbody>{members.map((m:any,ri:number)=>{const uid=m.user_id;const usr=m.users;const sc=getS(uid);const ans=hasA(uid);return(<tr key={uid} className="border-b border-[#0B132B]/5"><td className="sticky left-0 bg-white rounded-lg border border-[#D4AF37]/10 shadow-sm text-[#0B132B] z-10 px-3 py-2 font-semibold text-[#0B132B]">{usr?.login_id||usr?.name||"?"}</td><td className="px-2 py-2 text-center font-bold text-[#B59132]">{ans?sc:"미응시"}</td>{qs.map((q,ci:number)=>{const k=`${uid}-${q.question_number}`;const v=grid[k];const cellKey=`${ri}-${ci}`;const isSel=selCells.has(cellKey);return(<td key={q.question_number} className="px-0.5 py-1 text-center" onMouseDown={()=>onCellMouseDown(ri,ci)} onMouseEnter={()=>onCellMouseEnter(ri,ci)}><input data-grid-row={ri} data-grid-col={ci} readOnly className={`w-7 h-7 text-center rounded font-bold text-xs border cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#D4AF37] ${isSel?"ring-2 ring-[#D4AF37] border-[#D4AF37]":v===1?"bg-blue-50 border-blue-200 text-blue-600":v===0?"bg-red-50 border-red-200 text-red-500":"bg-white border-slate-100"}`} value={v===undefined?"":v} onKeyDown={e=>cellKeyDown(e,ri,ci,uid,q.question_number)} onFocus={e=>e.target.select()}/></td>);})}</tr>);})}</tbody></table>{selCells.size>1&&<div className="flex items-center gap-2 px-4 py-2 bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#B59132] border-t border-[#D4AF37]/10"><span className="text-xs text-[#B59132] font-semibold">{selCells.size}개 선택됨</span><button onClick={()=>applyToSelected("1")} className="bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg text-xs font-bold">전체 O</button><button onClick={()=>applyToSelected("0")} className="bg-red-50 text-red-500 px-2.5 py-1 rounded-lg text-xs font-bold">전체 X</button><button onClick={()=>{applyToSelected("");setSelCells(new Set());}} className="bg-slate-100 text-slate-500 px-2.5 py-1 rounded-lg text-xs font-bold">전체 삭제</button><button onClick={()=>setSelCells(new Set())} className="text-xs text-slate-400 ml-auto">선택 해제</button></div>}</div>
      <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm overflow-x-auto mb-4"><table className="text-xs border-collapse w-full"><thead><tr className="bg-slate-50 border border-slate-100 text-[#0B132B]"><th className="sticky left-0 bg-slate-50 border border-slate-100 text-[#0B132B] z-10 px-3 py-2 text-left font-semibold text-slate-500 min-w-[80px]">이름</th><th className="px-2 py-2 font-semibold text-slate-500">출석</th><th className="px-2 py-2 font-semibold text-slate-500">클리닉</th><th className="px-2 py-2 font-semibold text-slate-500">과제 성취도</th><th className="px-2 py-2 font-semibold text-slate-500">오답 성취도</th><th className="px-2 py-2 font-semibold text-slate-500 min-w-[280px]">개인 코멘트</th></tr></thead><tbody>{members.map((m:any)=>{const uid=m.user_id;const usr=m.users;const inf=ig[uid]||{};return(<tr key={uid} className="border-b border-[#0B132B]/5"><td className="sticky left-0 bg-white rounded-lg border border-[#D4AF37]/10 shadow-sm text-[#0B132B] z-10 px-3 py-2 font-semibold"><div className="flex items-center gap-1">{usr?.login_id||usr?.name||"?"}{(inf.attendance==="출석"||inf.attendance==="영상")&&<button onClick={()=>captureReport(uid)} className="text-[9px] text-slate-300 hover:text-[#B59132] bg-slate-50 border border-slate-100 text-[#0B132B] px-1.5 py-0.5 rounded">📷</button>}</div></td><td className="px-1 py-1"><select className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={inf.attendance||""} onChange={e=>setIC(uid,"attendance",e.target.value)}><option value="">—</option><option>출석</option><option>결석</option><option>영상</option></select></td><td className="px-1 py-1"><input className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={inf.clinic_time||""} onChange={e=>setIC(uid,"clinic_time",e.target.value)}/></td><td className="px-1 py-1"><input className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={inf.assignment_score||""} onChange={e=>setIC(uid,"assignment_score",e.target.value)}/></td><td className="px-1 py-1"><input className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={inf.wrong_answer_score||""} onChange={e=>setIC(uid,"wrong_answer_score",e.target.value)}/></td><td className="px-1 py-1"><textarea className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 w-full resize-none h-16" value={inf.comment||""} onChange={e=>setIC(uid,"comment",e.target.value)} placeholder="개인 코멘트"/></td></tr>);})}</tbody></table></div>
      <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm p-5"><h3 className="font-semibold text-sm mb-3">자동 계산 통계 (실시간)</h3><div className="grid grid-cols-3 gap-4 max-w-md"><div className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl p-3 text-center"><p className="text-[10px] text-slate-400">반 평균</p><p className="text-xl font-bold text-[#B59132]">{(Math.round(avg*10)/10).toFixed(1)}</p></div><div className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl p-3 text-center"><p className="text-[10px] text-slate-400">표준편차</p><p className="text-xl font-bold text-slate-600">{(Math.round(stdDev*10)/10).toFixed(1)}</p></div><div className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl p-3 text-center"><p className="text-[10px] text-slate-400">최고점</p><p className="text-xl font-bold text-slate-600">{best}</p></div></div></div>
      <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm p-5 mt-4"><div className="flex items-center justify-between mb-3"><h3 className="font-semibold text-sm">문항 설정</h3><div className="flex items-center gap-2"><label className="text-xs text-slate-400">문항수</label><input type="number" className="w-16 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-sm border-0 text-center font-semibold" value={qs.length} onChange={e=>{const v=Number(e.target.value);if(v>=1&&v<=50)changeQCount(v);}}/></div></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{qs.map(q=>(<div key={q.id} className="flex items-center gap-1.5"><span className="text-xs text-slate-400 w-5 text-right font-semibold">{q.question_number}</span><input className="flex-1 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0" defaultValue={q.topic||""} placeholder="단원명" onBlur={e=>saveTopic(q.id,e.target.value)}/></div>))}</div></div>
      {/* 숨겨진 성적표 캡쳐 영역 */}
  ...
  if(selG){
    ...
    return(<div>
      ...
      <div className="flex items-center gap-2 mb-4">{editingGId===selG.id?<div className="flex items-center gap-2"><input className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-1.5 text-lg font-bold border border-slate-100 focus:outline-none focus:border-[#D4AF37]" value={editGN} onChange={e=>setEditGN(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")renameG(selG.id);if(e.key==="Escape"){setEditingGId(null);setEditGN("");}}} autoFocus/><button onClick={()=>renameG(selG.id)} className="text-xs text-[#B59132] font-semibold bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-3 py-1.5 rounded-lg">확인</button><button onClick={()=>{setEditingGId(null);setEditGN("");}} className="text-xs text-slate-400">취소</button></div>:<><h2 className="text-lg font-bold">📁 {selG.name}</h2><button onClick={()=>{setEditingGId(selG.id);setEditGN(selG.name);}} className="text-xs text-slate-400 hover:text-[#B59132]">✏️ 수정</button></>}</div>
      <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4"><div className="flex justify-between items-center mb-3"><h3 className="font-semibold text-sm">학생 ({members.length}명)</h3><button onClick={()=>setShowAM(!showAM)} className="text-xs text-[#B59132] font-semibold">+ 학생 추가</button></div><div className="flex flex-wrap gap-2">{members.map((m:any)=>(<div key={m.id} className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-3 py-1.5 text-xs flex items-center gap-2"><span className="font-medium">{m.users?.login_id||m.users?.name}</span><button onClick={()=>rM(m.id)} className="text-red-300 hover:text-red-500">×</button></div>))}{members.length===0&&<p className="text-slate-400 text-xs">학생 추가 필요</p>}</div></div>
      {showAM&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4"><h3 className="font-semibold text-sm mb-3">학생 검색 / 추가</h3><div className="flex items-center gap-2 mb-3 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-3 py-2"><Icon type="search" size={16}/><input className="flex-1 bg-transparent text-sm border-0 focus:outline-none" value={searchM} onChange={e=>setSearchM(e.target.value)} placeholder="이름, 아이디 또는 학교로 검색"/></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-60 overflow-y-auto">{filtered.map((u:any)=>(<button key={u.id} onClick={()=>aM(u.id)} className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg p-3 text-left hover:bg-blue-50 text-xs"><p className="font-semibold">{u.login_id||u.name}</p><p className="text-slate-400">{u.school||""}</p></button>))}{filtered.length===0&&<p className="text-slate-400 text-xs col-span-3">결과 없음</p>}</div></div>}
      ...
      {showNT&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 flex gap-3 items-end"><div className="flex-1"><label className="text-xs font-semibold text-slate-500">시험 날짜</label><input type="date" className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={ntf.date} onChange={e=>setNtf(p=>({...p,date:e.target.value}))}/></div><button onClick={cT} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-5 py-2.5 rounded-xl text-xs font-semibold">생성</button><button onClick={()=>setShowNT(false)} className="text-xs text-slate-400 pb-1">취소</button></div>}
      <div className="space-y-2">{tests.map(t=>(<div key={t.id} className="bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] p-4 shadow-sm hover:ring-2 hover:ring-[#D4AF37]/30">
        {editTest?.id===t.id?<div className="space-y-3">
          <h3 className="font-semibold text-sm">✏️ 시험 수정</h3>
          <div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-semibold text-slate-500">날짜</label><input type="date" className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={editTF.date} onChange={e=>setEditTF(p=>({...p,date:e.target.value}))}/></div><div><label className="text-xs font-semibold text-slate-500">시험명</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={editTF.title} onChange={e=>setEditTF(p=>({...p,title:e.target.value}))}/></div></div>
          <div><label className="text-xs font-semibold text-slate-500">과제</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={editTF.assignment} onChange={e=>setEditTF(p=>({...p,assignment:e.target.value}))}/></div>
          <div className="flex gap-2"><button onClick={updateTest} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditTest(null)} className="text-xs text-slate-400">취소</button></div>
          ...
          <div className="flex items-center gap-2 ml-2"><button onClick={(e)=>{e.stopPropagation();setEditTest(t);setEditTF({date:t.date,title:t.title,assignment:t.assignment||""});}} className="text-xs text-slate-300 hover:text-[#B59132]">수정</button><button onClick={(e)=>{e.stopPropagation();deleteTest(t.id);}} className="text-xs text-slate-300 hover:text-red-500">삭제</button><Icon type="right" size={16}/></div>
        </div>}
      </div>))}{tests.length===0&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-8 shadow-sm text-center text-slate-400 text-sm">시험 추가 필요</div>}</div>
    </div>);
  }
  ...
  return(<div>
    ...
    {showNG&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 flex gap-3 items-end"><div className="flex-1"><label className="text-xs font-semibold text-slate-500">반 이름</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={newGN} onChange={e=>setNewGN(e.target.value)} placeholder="수학 정규반"/></div><button onClick={cG} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2.5 rounded-xl text-xs font-semibold">만들기</button></div>}
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{groups.map(g=>(<div key={g.id} className="bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] p-5 shadow-sm hover:ring-2 hover:ring-[#D4AF37]/20 cursor-pointer" onClick={()=>selectGroup(g)}><div className="flex justify-between items-start"><div className="flex items-center gap-2"><Icon type="folder" size={20}/>{editingGId===g.id?<div className="flex items-center gap-1" onClick={e=>e.stopPropagation()}><input className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1 text-sm border border-slate-100 focus:outline-none focus:border-[#D4AF37] w-36" value={editGN} onChange={e=>setEditGN(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")renameG(g.id);if(e.key==="Escape"){setEditingGId(null);setEditGN("");}}} autoFocus/><button onClick={()=>renameG(g.id)} className="text-xs text-[#B59132] font-semibold">확인</button><button onClick={()=>{setEditingGId(null);setEditGN("");}} className="text-xs text-slate-400">취소</button></div>:<span className="font-semibold">{g.name}</span>}</div><div className="flex gap-2" onClick={e=>e.stopPropagation()}><button onClick={()=>{setEditingGId(g.id);setEditGN(g.name);}} className="text-slate-300 hover:text-[#B59132] text-xs">수정</button><button onClick={()=>dG(g.id)} className="text-slate-300 hover:text-red-400 text-xs">삭제</button></div></div></div>))}{groups.length===0&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm col-span-2">반을 만들어 보세요!</div>}</div>
  </div>);
}
...
function AdminNoticeManager({groups}:{groups:any[]}){
  ...
  return(<div>
    ...
    {showAdd&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><label className="text-xs font-semibold text-slate-500">반 선택</label><select className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.class_group_id} onChange={e=>setForm(p=>({...p,class_group_id:Number(e.target.value)}))}><option value={0}>반을 선택하세요</option>{groups.map(g=>(<option key={g.id} value={g.id}>{g.name}</option>))}</select></div>
        <div><label className="text-xs font-semibold text-slate-500">제목</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} placeholder="공지 제목"/></div>
      </div>
      <div><label className="text-xs font-semibold text-slate-500">내용</label><textarea className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm mt-1 border-0 resize-none h-28" value={form.content} onChange={e=>setForm(p=>({...p,content:e.target.value}))} placeholder="공지 내용을 입력하세요"/></div>
      <div className="flex gap-2"><button onClick={addNotice} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">등록</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}
    <div className="space-y-3">{notices.map((n:any)=>(<div key={n.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm">
      <div className="flex items-start justify-between mb-2"><div><h3 className="font-semibold text-sm">{n.title||"공지"}</h3><div className="flex items-center gap-2 mt-1"><span className="text-xs text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2 py-0.5 rounded-lg">{n.class_groups?.name||""}</span><span className="text-xs text-slate-400">{n.created_at?.slice(0,10)}</span></div></div><button onClick={()=>delNotice(n.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div>
      <p className="text-sm text-slate-600 whitespace-pre-line">{n.content}</p>
    </div>))}{notices.length===0&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm">공지사항을 작성해보세요</div>}</div>
  </div>);
}
...
function AdminExamViewer({users}:{users:any[]}){
  ...
  return(<div>
    ...
    <div className="flex flex-wrap gap-2 mb-4">
      <div className="flex items-center gap-2 bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-3 py-2 shadow-sm"><Icon type="search" size={16}/><input className="bg-transparent text-sm border-0 focus:outline-none w-32" value={filter} onChange={e=>setFilter(e.target.value)} placeholder="이름 검색"/></div>
      <select className="bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] px-3 py-2 text-sm shadow-sm border-0" value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="">전체</option><option>모의고사</option><option>내신</option></select>
    </div>
    {filtered.length>0?<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-50 border border-slate-100 text-[#0B132B]">{["이름","학교","유형","구분","과목","점수","등급","성적변화","고민","하고싶은말"].map(h=><th key={h} className="px-3 py-3 text-left text-xs font-semibold text-slate-400 whitespace-nowrap">{h}</th>)}</tr></thead><tbody>{filtered.map((e:any)=>{let memoObj:any={};try{memoObj=JSON.parse(e.memo||"{}");}catch{}return(<tr key={e.id} className="border-t border-[#0B132B]/5 hover:bg-[#D4AF37]/10"><td className="px-3 py-2.5 font-semibold whitespace-nowrap">{e.users?.name||"?"}</td><td className="px-3 py-2.5 text-xs text-slate-500">{e.users?.school||"—"}</td><td className="px-3 py-2.5"><span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${e.exam_type==="모의고사"?"bg-amber-50 text-amber-600":"bg-blue-50 text-blue-600"}`}>{e.exam_type}</span></td><td className="px-3 py-2.5 text-xs">{e.exam_name||"—"}</td><td className="px-3 py-2.5 text-xs">{e.subject}</td><td className="px-3 py-2.5 font-bold text-[#B59132]">{e.score}점</td><td className="px-3 py-2.5 text-xs">{e.grade||"—"}</td><td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px]">{memoObj.q1||"—"}</td><td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px]">{memoObj.q2||"—"}</td><td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px]">{memoObj.q3||"—"}</td></tr>);})}</tbody></table></div>:<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm">학생이 입력한 성적이 없습니다</div>}
  </div>);
}
...
function AdminInquiryManager({onReply}:{onReply?:()=>void}){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">💬 문의사항 관리</h2>{inqs.length>0?<div className="space-y-3">{inqs.map((q:any)=>{const isNew=q.created_at&&(Date.now()-new Date(q.created_at).getTime())<24*60*60*1000;const hasReply=!!q.reply;const imgMatch=q.content?.match(/\[IMG\](.*?)\[\/IMG\]/);const cleanContent=q.content?.replace(/\[IMG\].*?\[\/IMG\]/g,"").trim();const replyImgMatch=q.reply?.match(/\[IMG\](.*?)\[\/IMG\]/);const cleanReply=q.reply?.replace(/\[IMG\].*?\[\/IMG\]/g,"").trim();return(<div key={q.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm">
    <div className="flex justify-between mb-2"><div className="flex items-center gap-2"><span className="text-xs font-bold text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2 py-0.5 rounded-lg">{q.users?.name||"?"}</span><span className="text-xs text-slate-400">{q.created_at?.slice(0,10)}</span>{isNew&&<span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">N</span>}<span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${hasReply?"bg-green-50 text-green-600":"bg-amber-50 text-amber-600"}`}>{hasReply?"답변 완료":"답변 대기중"}</span></div><button onClick={()=>delInq(q.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div>
    ...
    {hasReply?<div className="bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#B59132] rounded-xl p-3"><p className="text-xs font-semibold text-[#B59132] mb-1">내 답변</p><p className="text-sm text-[#0B132B] whitespace-pre-line">{cleanReply}</p>{replyImgMatch&&<img src={replyImgMatch[1]} alt="" className="mt-2 rounded-xl max-h-48 object-contain"/>}<button onClick={()=>{setReplyId(q.id);setReplyText(cleanReply||"");}} className="text-xs text-slate-400 mt-2">수정</button></div>:replyId===q.id?<div className="space-y-2"><textarea className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm border-0 resize-none h-20" value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="답변을 작성하세요"/><div className="flex items-center gap-2"><button onClick={()=>replyImgRef.current?.click()} className="bg-slate-50 border border-slate-100 text-[#0B132B] border border-slate-100 rounded-xl px-3 py-1.5 text-xs text-slate-500">{replyImg?replyImg.name:"이미지 첨부"}</button><input ref={replyImgRef} type="file" accept="image/*" className="hidden" onChange={e=>{if(e.target.files?.[0])setReplyImg(e.target.files[0]);}}/>{replyImg&&<button onClick={()=>setReplyImg(null)} className="text-xs text-red-400">삭제</button>}</div><div className="flex gap-2"><button onClick={()=>saveReply(q.id)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-1.5 rounded-xl text-xs font-semibold">답변</button><button onClick={()=>{setReplyId(null);setReplyImg(null);}} className="text-xs text-slate-400">취소</button></div></div>:<button onClick={()=>{setReplyId(q.id);setReplyText("");}} className="text-xs text-[#B59132] font-semibold">답변하기</button>}
  </div>);})}</div>:<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm">문의가 없습니다</div>}</div>);
}
...
function AdminTokenManager({users,fetchUsers}:{users:any[];fetchUsers:()=>void}){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">🥩 서서갈비 관리</h2>
    <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm overflow-x-auto mb-6"><table className="w-full text-sm"><thead><tr className="bg-slate-50 border border-slate-100 text-[#0B132B]"><th className="px-4 py-3 text-left text-xs font-semibold text-slate-400">이름</th><th className="px-4 py-3 text-xs font-semibold text-slate-400">보유</th><th className="px-4 py-3 text-xs font-semibold text-slate-400">수량</th><th className="px-4 py-3 text-xs font-semibold text-slate-400">사유</th><th className="px-4 py-3"></th></tr></thead><tbody>{students.map((s:any)=>(<tr key={s.id} className="border-t border-[#0B132B]/5"><td className="px-4 py-2.5 font-semibold">{s.name}</td><td className="px-4 py-2.5 text-center font-bold text-[#B59132]">{s.tokens||0} 🥩</td><td className="px-4 py-2.5"><input type="number" className="w-20 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 text-center" value={amount[s.id]||""} onChange={e=>setAmount(p=>({...p,[s.id]:e.target.value}))} placeholder="0"/></td><td className="px-4 py-2.5"><input className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-lg px-2 py-1.5 text-xs border-0 w-full" value={reason[s.id]||""} onChange={e=>setReason(p=>({...p,[s.id]:e.target.value}))} placeholder="사유"/></td><td className="px-4 py-2.5"><div className="flex gap-1"><button onClick={()=>giveToken(s.id)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-3 py-1 rounded-lg text-xs font-semibold">지급</button><button onClick={()=>giveToken(s.id,true)} className="bg-red-50 text-red-500 px-3 py-1 rounded-lg text-xs font-semibold">차감</button></div></td></tr>))}</tbody></table></div>
    <h3 className="font-semibold text-sm mb-3">지급/차감 내역</h3>
    <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] shadow-sm overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-50 border border-slate-100 text-[#0B132B]"><th className="px-4 py-3 text-left text-xs font-semibold text-slate-400">날짜</th><th className="px-4 py-3 text-xs font-semibold text-slate-400">이름</th><th className="px-4 py-3 text-xs font-semibold text-slate-400">수량</th><th className="px-4 py-3 text-xs font-semibold text-slate-400">사유</th></tr></thead><tbody>{logs.map((l:any)=>(<tr key={l.id} className="border-t border-[#0B132B]/5"><td className="px-4 py-2 text-xs text-slate-400">{l.created_at?.slice(0,10)}</td><td className="px-4 py-2 text-sm font-semibold">{userMap[l.user_id]||"?"}</td><td className={`px-4 py-2 text-sm font-bold text-center ${l.amount>0?"text-[#B59132]":"text-red-500"}`}>{l.amount>0?"+":""}{l.amount} 🥩</td><td className="px-4 py-2 text-xs text-slate-500">{l.reason||"—"}</td></tr>))}{logs.length===0&&<tr><td colSpan={4} className="text-center py-8 text-slate-400 text-sm">내역 없음</td></tr>}</tbody></table></div>
  </div>);
}
...
function AdminShopManager({onProcess}:{onProcess?:()=>void}){
  ...
  return(<div><div className="flex justify-between items-center mb-4"><h2 className="text-lg font-bold">🏪 상점 관리</h2><button onClick={()=>setShowAdd(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-1 shadow-md shadow-[#D4AF37]/20 hover:shadow-lg transition-all"><Icon type="plus" size={14}/>상품 추가</button></div>
    {showAdd&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 space-y-3"><div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-semibold text-slate-500">상품명</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div><div><label className="text-xs font-semibold text-slate-500">가격 (서서갈비)</label><input type="number" className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.price} onChange={e=>setForm(p=>({...p,price:Number(e.target.value)}))}/></div></div><div><label className="text-xs font-semibold text-slate-500">설명</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))}/></div><div className="flex gap-2"><button onClick={addItem} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">추가</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div></div>}
    ...
    <div className="space-y-2 mb-6">{items.map((item:any)=>(<div key={item.id} className={`bg-white rounded-xl border border-[#D4AF37]/15 shadow-sm text-[#0B132B] p-4 shadow-sm flex justify-between items-center ${!item.active?"opacity-50":""}`}><div><p className="font-semibold text-sm">{item.name}</p><p className="text-xs text-slate-400">{item.description||"설명 없음"} · 🥩 {item.price}</p></div><div className="flex gap-2"><button onClick={()=>toggleItem(item.id,item.active)} className="text-xs text-slate-400">{item.active?"비활성":"활성"}</button><button onClick={()=>delItem(item.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div></div>))}{items.length===0&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm">상품을 추가하세요</div>}</div>

    {/* 처리 완료 내역 */}
    {done.length>0&&<div><h3 className="font-semibold text-sm mb-3">처리 완료 내역</h3><div className="space-y-2">{done.slice(0,20).map((p:any)=>(<div key={p.id} className="bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 flex justify-between items-center"><div><p className="text-sm">{p.users?.name||"?"} — {p.shop_items?.name||""}</p><p className="text-[10px] text-slate-400">{p.created_at?.slice(0,10)}</p></div><span className="text-xs text-green-500 font-semibold">✓ 완료</span></div>))}</div></div>}
  </div>);
}
...
function ShortsGrid(){
  ...
  if(shorts.length===0)return(<div className="bg-white border border-[#D4AF37]/10 shadow-sm rounded-2xl p-12 border border-[#0B132B]/5 text-center text-slate-400 text-sm">영상 준비 중입니다</div>);
  return(<>{playing&&<div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={()=>setPlaying(null)}><div className="relative w-full max-w-sm" style={{aspectRatio:"9/16"}} onClick={e=>e.stopPropagation()}><iframe src={`https://www.youtube.com/embed/${playing}?autoplay=1&loop=1`} className="w-full h-full rounded-2xl" allow="autoplay; encrypted-media" allowFullScreen/><button onClick={()=>setPlaying(null)} className="absolute -top-3 -right-3 bg-white rounded-lg border border-[#D4AF37]/10 shadow-sm text-[#0B132B] w-8 h-8 rounded-full shadow-lg flex items-center justify-center text-slate-600 hover:text-red-500 transition-colors"><Icon type="close" size={16}/></button></div></div>}
  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">{shorts.map((s:any)=>(<button key={s.id} onClick={()=>setPlaying(s.video_id)} className="group relative bg-black rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all hover:scale-[1.02]" style={{aspectRatio:"9/16"}}><img src={`https://img.youtube.com/vi/${s.video_id}/0.jpg`} alt={s.title} className="absolute inset-0 w-full h-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent"/><div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><div className="w-12 h-12 bg-white rounded-lg border border-[#D4AF37]/10 shadow-sm text-[#0B132B]/90 rounded-full flex items-center justify-center shadow-lg"><svg viewBox="0 0 24 24" width="20" height="20" fill="#0B132B"><polygon points="8 5 20 12 8 19"/></svg></div></div>{s.title&&<p className="absolute bottom-2 left-2 right-2 text-[10px] text-white font-semibold leading-tight line-clamp-2">{s.title}</p>}</button>))}</div></>);
}
...
function AdminReviewViewer(){
  ...
  return(<div><div className="flex justify-between items-center mb-4"><h2 className="text-lg font-bold">✍️ 후기 관리 (로그인 화면 노출용)</h2><button onClick={()=>setShowAdd(true)} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold px-4 py-2 rounded-xl text-xs font-semibold shadow-md shadow-[#D4AF37]/20 flex items-center gap-1"><Icon type="plus" size={14}/>후기 추가</button></div>
    {showAdd&&<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 space-y-3">
      <div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-semibold text-slate-500">학생 이름 *</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.display_name} onChange={e=>setForm(p=>({...p,display_name:e.target.value}))} placeholder="예: 김OO"/></div><div><label className="text-xs font-semibold text-slate-500">학교</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.display_school} onChange={e=>setForm(p=>({...p,display_school:e.target.value}))} placeholder="예: 서서고"/></div></div>
      <div><label className="text-xs font-semibold text-slate-500">성적 향상</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={form.best_grade} onChange={e=>setForm(p=>({...p,best_grade:e.target.value}))} placeholder="예: 500등 → 100등"/></div>
      <div><label className="text-xs font-semibold text-slate-500">장점 키워드 (쉼표 구분)</label><div className="flex flex-wrap gap-1.5 mt-1">{kwOptions.map(kw=>{const sel=form.keywords.split(",").map(k=>k.trim()).includes(kw);return(<button key={kw} onClick={()=>setForm(p=>({...p,keywords:kwToggle(p.keywords,kw)}))} className={`px-2.5 py-1 rounded-full text-[10px] font-semibold transition-all ${sel?"bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)]":"bg-slate-100 text-slate-500"}`}>#{kw}</button>);})}</div></div>
      <div><label className="text-xs font-semibold text-slate-500">수강 후기 *</label><textarea className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm mt-1 border-0 resize-none h-24" value={form.content} onChange={e=>setForm(p=>({...p,content:e.target.value}))} placeholder="후기 내용"/></div>
      <div className="flex gap-2"><button onClick={addReview} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">추가</button><button onClick={()=>setShowAdd(false)} className="text-xs text-slate-400">취소</button></div>
    </div>}
    <p className="text-xs text-slate-400 mb-3">아래 후기들이 로그인 화면 하단 슬라이더에 표시됩니다.</p>
    {reviews.length>0?<div className="space-y-3">{reviews.map((r:any)=>(<div key={r.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm">
      {editId===r.id?<div className="space-y-3">
        <div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-semibold text-slate-500">학생 이름 *</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={editForm.display_name} onChange={e=>setEditForm(p=>({...p,display_name:e.target.value}))}/></div><div><label className="text-xs font-semibold text-slate-500">학교</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={editForm.display_school} onChange={e=>setEditForm(p=>({...p,display_school:e.target.value}))}/></div></div>
        <div><label className="text-xs font-semibold text-slate-500">성적 향상</label><input className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm mt-1 border-0" value={editForm.best_grade} onChange={e=>setEditForm(p=>({...p,best_grade:e.target.value}))} placeholder="예: 500등 → 100등"/></div>
        ...
        <div><label className="text-xs font-semibold text-slate-500">수강 후기 *</label><textarea className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm mt-1 border-0 resize-none h-24" value={editForm.content} onChange={e=>setEditForm(p=>({...p,content:e.target.value}))}/></div>
        <div className="flex gap-2"><button onClick={saveEdit} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-4 py-2 rounded-xl text-xs font-semibold">저장</button><button onClick={()=>setEditId(null)} className="text-xs text-slate-400">취소</button></div>
      </div>:<>
        <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><span className="text-xs font-bold text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2 py-0.5 rounded-lg">{r.display_name||"?"}</span><span className="text-xs text-slate-400">{r.display_school||""}</span></div><div className="flex items-center gap-2"><button onClick={()=>startEdit(r)} className="text-xs text-slate-300 hover:text-[#B59132]">수정</button><button onClick={()=>delReview(r.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div></div>
        {r.best_grade&&<p className="text-xs text-amber-600 font-semibold mb-1">📈 {r.best_grade}</p>}
        {r.keywords&&<div className="flex flex-wrap gap-1 mb-2">{r.keywords.split(",").map((kw:string)=>(<span key={kw} className="bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 text-[#B59132] px-2 py-0.5 rounded-full text-[10px] font-semibold">#{kw}</span>))}</div>}
        <p className="text-sm text-slate-600 whitespace-pre-line">{r.content}</p>
      </>}
    </div>))}</div>:<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm">후기를 추가하면 로그인 화면에 표시됩니다</div>}
  </div>);
}
...
function AdminStudentReviewViewer(){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">📝 학생 후기 ({reviews.length}건)</h2>
    {reviews.length>0?<div className="space-y-3">{reviews.map((r:any)=>(<div key={r.id} className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm"><div className="flex items-center gap-2 mb-3"><span className="text-xs font-bold text-[#B59132] bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 px-2 py-0.5 rounded-lg">{r.users?.login_id||r.users?.name||"?"}</span><span className="text-xs text-slate-400">{r.users?.school||""}</span><span className="text-xs text-slate-300 ml-auto">{r.created_at?.slice(0,10)}</span><button onClick={()=>delStudentReview(r.id)} className="text-xs text-slate-300 hover:text-red-500">삭제</button></div>
      {r.best_grade&&<div className="mb-2"><p className="text-[10px] font-semibold text-slate-400">📈 성적 향상</p><p className="text-sm font-bold text-[#0B132B]">{r.best_grade}</p></div>}
    ...
    </div>))}</div>:<div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-12 shadow-sm text-center text-slate-400 text-sm">학생이 작성한 후기가 없습니다</div>}
  </div>);
}
...
function AdminShortsManager(){
  ...
  return(<div><h2 className="text-lg font-bold mb-4">▶ 쇼츠 관리</h2>
    <div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-5 shadow-sm mb-4 space-y-3"><p className="text-xs text-slate-400">유튜브 쇼츠 URL을 붙여넣기</p><div className="flex gap-2"><input className="flex-1 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm border-0" value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://youtube.com/shorts/xxxxx"/></div><div className="flex gap-2"><input className="flex-1 bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-2.5 text-sm border-0" value={title} onChange={e=>setTitle(e.target.value)} placeholder="제목 (선택)"/><button onClick={addShort} className="bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)] px-5 py-2.5 rounded-xl text-xs font-semibold">추가</button></div></div>
...
export default function Home(){
  ...
  if(initializing)return<div className="min-h-screen bg-[#FDFBF7] text-[#0B132B] flex items-center justify-center"><img src="/logo.png" alt="" className="h-10 opacity-50 animate-pulse"/></div>;
  if(!user)return<LoginScreen onLogin={handleLogin} settings={settings}/>;
  if(loading)return<div className="min-h-screen bg-[#FDFBF7] text-[#0B132B] flex items-center justify-center"><img src="/logo.png" alt="" className="h-10 opacity-50 animate-pulse"/></div>;
  ...
  const navEl=(mob?:boolean)=>(<nav className={`${mob?"":"flex-1"} space-y-1`}>
    {miPublic.map(m=>(<button key={m.id} onClick={()=>handleAdminTab(m.id,mob)} className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium transition-colors relative ${tab===m.id?"bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] font-bold shadow-[0_4px_15px_rgba(212,175,55,0.3)]":"text-slate-500 hover:bg-[#D4AF37]/10"}`}><Icon type={m.icon} size={18}/>{m.label}</button>))}
    <div className="pt-2 mt-2 border-t border-slate-100">
      <button onClick={()=>{if(adminUnlocked){setAdminUnlocked(false);setAdminPwInput("");setTab("classes");if(mob)setMm(false);}else{setTab("unlock");if(mob)setMm(false);}}} className="flex items-center gap-2 w-full px-3 py-1.5 text-[10px] font-semibold text-slate-400 mb-1">{adminUnlocked?"🔓 관리 메뉴 (잠그기)":"🔒 관리 메뉴 (잠김)"}</button>
  ...
  return(<div className="min-h-screen bg-[#FDFBF7] text-[#0B132B] flex">
    <aside className="hidden lg:flex flex-col w-56 bg-white shadow-[4px_0_24px_rgba(0,0,0,0.02)] border-r border-[#D4AF37]/10 min-h-screen p-5 fixed left-0 top-0 bottom-0 z-40"><div className="flex items-center gap-3 mb-8"><img src="/logo.png" alt="" className="h-7 object-contain"/><span className="font-bold text-slate-800 text-sm">서정인 수학</span></div>{navEl()}<div className="pt-4 border-t border-slate-100 mt-4"><div className="flex items-center gap-3 mb-3 px-1"><div className="w-8 h-8 bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 rounded-full flex items-center justify-center text-[#B59132]"><Icon type="user" size={14}/></div><div><p className="text-xs font-semibold">{user.name}</p><p className="text-[10px] text-slate-400">관리자</p></div></div><button onClick={logout} className="flex items-center gap-2 w-full px-3 py-2 rounded-xl text-sm text-slate-400 hover:text-red-400 hover:bg-red-50"><Icon type="logout" size={16}/>로그아웃</button></div></aside>
    <div className="lg:hidden fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-b border-[#D4AF37]/10 shadow-md z-40 text-[#0B132B] px-4 py-3 flex justify-between items-center"><div className="flex items-center gap-2"><img src="/logo.png" alt="" className="h-6 object-contain"/><span className="font-bold text-sm">서정인 수학</span></div><button onClick={()=>setMm(!mm)}><Icon type={mm?"close":"menu"} size={22}/></button></div>
    {mm&&<><div onClick={()=>setMm(false)} className="lg:hidden fixed inset-0 bg-slate-50 z-40"/><div className="lg:hidden fixed right-0 top-0 bottom-0 w-64 bg-white/95 backdrop-blur-3xl z-50 border-r border-[#D4AF37]/20 text-[#0B132B] p-5 shadow-xl"><div className="flex justify-between items-center mb-6"><span className="font-bold">메뉴</span><button onClick={()=>setMm(false)}><Icon type="close" size={20}/></button></div>{navEl(true)}<button onClick={()=>{logout();setMm(false);}} className="flex items-center gap-2 w-full px-3 py-2.5 rounded-xl text-sm text-red-400 mt-3"><Icon type="logout" size={16}/>로그아웃</button></div></>}
    <main className="flex-1 lg:ml-56 pt-16 lg:pt-0"><div className="max-w-5xl mx-auto p-5 lg:p-8">
      {tab==="unlock"&&<div className="max-w-sm mx-auto mt-20"><div className="bg-white rounded-2xl border border-[#D4AF37]/15 shadow-[0_8px_30px_rgba(0,0,0,0.03)] text-[#0B132B] p-8 shadow-sm text-center"><div className="w-16 h-16 bg-[#D4AF37]/15 text-[#B59132] border border-[#D4AF37]/30 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">🔒</div><h2 className="text-lg font-bold mb-1">관리 메뉴</h2><p className="text-xs text-slate-400 mb-6">접근하려면 비밀번호를 입력하세요</p><input type="password" className="w-full bg-slate-50 border border-slate-100 text-[#0B132B] rounded-xl px-4 py-3 text-sm border-0 text-center mb-3" value={adminPwInput} onChange={e=>{setAdminPwInput(e.target.value);setAdminPwErr("");}} placeholder="비밀번호" onKeyDown={e=>e.key==="Enter"&&tryUnlock()}/>{adminPwErr&&<p className="text-xs text-red-400 mb-3">{adminPwErr}</p>}<button onClick={tryUnlock} className="w-full bg-gradient-to-r from-[#0B132B] to-[#1A233A] text-[#D4AF37] shadow-[0_4px_15px_rgba(212,175,55,0.3)] font-bold py-3 rounded-xl font-semibold text-sm shadow-md shadow-[#D4AF37]/20">확인</button></div></div>}
